...
def test_main():
    ...
    FEM_PARAMS = {"order_disp": 1, "quad_degree": 5, "incompressible_2field": False}

    MATERIALS = {
        "MAT1": {"neohooke_compressible": {"mu": 10.0, "nu": 0.49}},
    ...
    class expression1:
        ...
        def evaluate(self, x):
            val = 0.5 * self.t
            return (np.full(x.shape[1], val), np.full(x.shape[1], val), np.full(x.shape[1], val))

    BC_DICT = {
        "membrane": [
            {
                "id": [5],
                "params": {"model": "membrane", "a_0": 1.0, "b_0": 6.0, "eta": 0.0, "rho0": 0.0, "h0": {"val": 0.1}},
            },  # block1
            {
                "id": [11],
                "params": {"model": "membrane", "a_0": 1.0, "b_0": 6.0, "eta": 0.0, "rho0": 0.0, "h0": {"val": 0.1}},
