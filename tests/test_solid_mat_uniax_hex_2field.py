...
def test_main():
    ...
    IO_PARAMS = {
        ...
        "results_to_write": [
            "displacement",
            "cauchystress",
            "cauchystress_principal",
            "vonmises_cauchystress",
            "pk1stress",
            "pk2stress",
            "glstrain",
            "glstrain_principal",
    ...
    def P_nh(lam):
        ...
        return mu * (lam - 1.0 / lam**2.0)

    # Mooney Rivlin
    def P_mr(lam):
        c1, c2 = MATERIALS["MAT2"]["mooneyrivlin_dev"]["c1"], MATERIALS["MAT2"]["mooneyrivlin_dev"]["c2"]
        return 2.0 * c1 * (lam - 1.0 / (lam**2.0)) + 2.0 * c2 * (1.0 - 1.0 / (lam**3.0))
    ...
    def P_ho(lam):
        ...
        return a_0 * (lam - 1.0 / lam**2.0) * np.exp(b_0 * (lam**2.0 + 2.0 / lam - 3.0)) + a_f * 2.0 * lam * (
            lam**2.0 - 1
        ) * np.exp(b_f * (lam**2.0 - 1.0) ** 2.0)
