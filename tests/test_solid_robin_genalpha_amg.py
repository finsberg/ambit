...
def test_main():
    ...
    MATERIALS = {"MAT1": {"stvenantkirchhoff": {"Emod": 1000.0, "nu": 0.3}, "inertia": {"rho0": 1.0e-6}}}
    ...
    class expression1:
        ...
        def evaluate(self, x):
            val = 3.0 * self.t
            return (np.full(x.shape[1], val), np.full(x.shape[1], 0.0), np.full(x.shape[1], 0.0))

    BC_DICT = {
        "neumann": [{"id": [3], "dir": "xyz_ref", "expression": expression1}],
