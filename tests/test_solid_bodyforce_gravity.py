...
def test_main():
    ...
    TIME_PARAMS = {"maxtime": 1.0, "numstep": 10, "timint": "genalpha", "rho_inf_genalpha": 1.0}
    ...
    check_node = []
    check_node.append(np.array([-1.0000000000000000e00, 1.0000000000000000e00, 1.0000000000000000e01]))
    ...
    check1 = ambit_fe.resultcheck.results_check_node(
        problem.mp.u, check_node, u_corr, problem.mp.V_u, problem.mp.comm, tol=tol_u, nm="u"
    )
    check2 = ambit_fe.resultcheck.results_check_node(
        problem.mp.io.v_proj, check_node, v_corr, problem.mp.V_u, problem.mp.comm, tol=tol_v, nm="v"
    )
    check3 = ambit_fe.resultcheck.results_check_node(
        problem.mp.io.a_proj, check_node, a_corr, problem.mp.V_u, problem.mp.comm, tol=tol_a, nm="a"
    )
