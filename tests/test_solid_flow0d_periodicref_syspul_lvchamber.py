...
def test_main():
    ...
    try:
        restart_step_outer = int(sys.argv[2])
    except:
        restart_step_outer = 0

    IO_PARAMS = {
        "problem_type": "solid_flow0d_periodicref",
    ...
    TIME_PARAMS = {"maxtime": 1.0, "numstep": 100, "timint": "genalpha", "rho_inf_genalpha": 0.8}
    ...
    TIME_PARAMS_FLOW0D = {
        ...
        "eps_periodic": 0.25,
    }  # large, only for testing purposes
    ...
    COUPLING_PARAMS = {
        ...
        "restart_periodicref": restart_step_outer,
    ...
    class time_curves:
        ...
        def tc2(self, t):  # RV
            ...
            return (
                0 * (t < t_ed)
                + act_rv_up * (t >= t_ed) * (t < (t_ed + tvu))
                + (1.0) * (t >= (t_ed + tvu)) * (t < (t_es))
                + act_rv_down * (t >= t_es) * (t < (t_es + tvr))
                + 0 * (t > (t_es + tvr))
            )
        ...
        def tc3(self, t):  # RA
            ...
            return 0.5 * (1.0 - np.cos(2.0 * np.pi * (t) / (2.0 * t_ed))) * (t >= 0.0) * (t <= 2.0 * t_ed)
    ...
    class expression1:  # prestress load
        ...
        def evaluate(self, x):
            pres = -0.5 * (self.val) * (1.0 - np.cos(np.pi * self.t / 1.0))
            return np.full(x.shape[1], pres)

    BC_DICT = {
        "dirichlet": [{"id": [1], "dir": "y", "val": 0.0}, {"id": [1], "dir": "z", "val": 0.0}],
        "robin": [
            {"type": "spring", "id": [2, 3, 4, 5, 6, 7], "dir": "xyz_ref", "stiff": 0.5},
    ...
    # correct results
    s_corr[0] = 6.9421708117080034e03
    s_corr[1] = 4.4439462150660586e-01
    s_corr[2] = -6.9429701758124638e03
    s_corr[3] = -7.9936410446000439e-01
    s_corr[4] = 8.4310934952949417e00
    s_corr[5] = -7.9936410446000439e-01
    s_corr[6] = 8.4310982914795680e00
    s_corr[7] = 5.1723054532288545e04
    s_corr[8] = 2.2450224403302048e00
    s_corr[9] = 7.0956502600423570e04
    s_corr[10] = 3.6548443149263097e04
    s_corr[11] = 5.4206637792003998e-01
    s_corr[12] = -1.2050357157719344e-01
    s_corr[13] = 5.0551793477077689e-01
    s_corr[14] = 1.7105536505427112e00
    s_corr[15] = 2.6480887501748493e04
    s_corr[16] = 1.3133403380164841e00
    s_corr[17] = 5.7929714433991918e04
