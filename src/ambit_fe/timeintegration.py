...
class timeintegration:
    def __init__(self, time_params, dt, Nmax, time_curves=None, t_init=0.0, dim=3, comm=None):
        try:
        ...
        try:
            self.eval_nonlin_terms = time_params["eval_nonlin_terms"]
        except:
            self.eval_nonlin_terms = "trapezoidal"
    ...
    def print_prestress_step(self, N, t, Nmax, lsp, ni=0, li=0, wt=0):
        msg = "### PRESTRESS STEP %i / %i completed | PSEUDO TIME: %.4f | ni = %i | li = %i | wt = %.2e" % (
            N,
            Nmax,
            t,
            ni,
    ...
    def set_time_funcs(self, t, dt, midp=False):
        for m in self.funcs_to_update_vec:
            ...
            if "funcs_mult" in m.keys():
                ...
                m["funcs_mult"][1].x.petsc_vec.pointwiseMult(
                    list(m.keys())[0].x.petsc_vec, m["funcs_mult"][0].x.petsc_vec
                )
        ...
        if midp:
            ...
            for m in self.funcs_to_update_vec_mid:
                load = expression.template_vector(dim=self.dim)
                load.val_x, load.val_y, load.val_z = (
                    list(m.values())[0][0](tmid),
                    list(m.values())[0][1](tmid),
                    list(m.values())[0][2](tmid),
                )
                list(m.keys())[0].interpolate(load.evaluate)
                list(m.keys())[0].x.petsc_vec.ghostUpdate(addv=PETSc.InsertMode.INSERT, mode=PETSc.ScatterMode.FORWARD)
    ...
    def update_time_funcs_old(self):
        ...
        for m in range(len(self.funcs_to_update_vec_old)):
            if list(self.funcs_to_update_vec_old[m].keys())[0] is not None:
                list(self.funcs_to_update_vec_old[m].keys())[0].x.petsc_vec.axpby(
                    1.0, 0.0, list(self.funcs_to_update_vec[m].keys())[0].x.petsc_vec
                )
                list(self.funcs_to_update_vec_old[m].keys())[0].x.petsc_vec.ghostUpdate(
                    addv=PETSc.InsertMode.INSERT, mode=PETSc.ScatterMode.FORWARD
                )

        for m in range(len(self.funcs_to_update_old)):
            if list(self.funcs_to_update_old[m].keys())[0] is not None:
                list(self.funcs_to_update_old[m].keys())[0].x.petsc_vec.axpby(
                    1.0, 0.0, list(self.funcs_to_update[m].keys())[0].x.petsc_vec
                )
                list(self.funcs_to_update_old[m].keys())[0].x.petsc_vec.ghostUpdate(
                    addv=PETSc.InsertMode.INSERT, mode=PETSc.ScatterMode.FORWARD
                )
    ...
    def update_dvar_ost(self, var, var_old, dvar_old, dt, dvarout=None, uflform=True):
        if uflform:  # ufl form
            return 1.0 / (self.theta_ost * dt) * (var - var_old) - (1.0 - self.theta_ost) / self.theta_ost * dvar_old
        else:  # PETSc vector update
            dvarout.axpby(-(1.0 - self.theta_ost) / self.theta_ost, 0.0, dvar_old)
            dvarout.axpy(1.0 / (self.theta_ost * dt), var)
            dvarout.axpy(-1.0 / (self.theta_ost * dt), var_old)
    ...
    def update_d2var_ost(self, var, var_old, dvar_old, d2var_old, dt, d2varout=None, uflform=True):
        if uflform:  # ufl form
            return (
                1.0 / (self.theta_ost * self.theta_ost * dt * dt) * (var - var_old)
                - 1.0 / (self.theta_ost * self.theta_ost * dt) * dvar_old
                - (1.0 - self.theta_ost) / self.theta_ost * d2var_old
            )
        else:  # PETSc vector update
            d2varout.axpby(-(1.0 - self.theta_ost) / self.theta_ost, 0.0, d2var_old)
            d2varout.axpy(-1.0 / (self.theta_ost * self.theta_ost * dt), dvar_old)
            d2varout.axpy(1.0 / (self.theta_ost * self.theta_ost * dt * dt), var)
            d2varout.axpy(-1.0 / (self.theta_ost * self.theta_ost * dt * dt), var_old)
    ...
    def update_dvar_newmark_1st(self, var, var_old, dvar_old, dt, dvarout=None, uflform=True):
        if uflform:  # ufl form
            return 1.0 / (self.gamma * dt) * (var - var_old) - (1.0 - self.gamma) / self.gamma * dvar_old
        else:  # PETSc vector update
            dvarout.axpby(-(1.0 - self.gamma) / self.gamma, 0.0, dvar_old)
            dvarout.axpy(1.0 / (self.gamma * dt), var)
            dvarout.axpy(-1.0 / (self.gamma * dt), var_old)
    ...
    def update_dvar_newmark_2nd(self, var, var_old, dvar_old, d2var_old, dt, dvarout=None, uflform=True):
        if uflform:  # ufl form
            return (
                self.gamma / (self.beta * dt) * (var - var_old)
                - (self.gamma - self.beta) / self.beta * dvar_old
                - (self.gamma - 2.0 * self.beta) / (2.0 * self.beta) * dt * d2var_old
            )
        else:  # PETSc vector update
            dvarout.axpby(-(self.gamma - self.beta) / self.beta, 0.0, dvar_old)
            dvarout.axpy(-(self.gamma - 2.0 * self.beta) / (2.0 * self.beta) * dt, d2var_old)
    ...
    def update_d2var_newmark(self, var, var_old, dvar_old, d2var_old, dt, d2varout=None, uflform=True):
        if uflform:  # ufl form
            return (
                1.0 / (self.beta * dt * dt) * (var - var_old)
                - 1.0 / (self.beta * dt) * dvar_old
                - (1.0 - 2.0 * self.beta) / (2.0 * self.beta) * d2var_old
            )
        else:  # PETSc vector update
            d2varout.axpby(-(1.0 - 2.0 * self.beta) / (2.0 * self.beta), 0.0, d2var_old)
            d2varout.axpy(-1.0 / (self.beta * dt), dvar_old)
    ...
    def update_varint_ost(self, var, var_old, varint_old, dt, varintout=None, uflform=True):
        if uflform:  # ufl form
            return self.theta_ost * dt * var + (1.0 - self.theta_ost) * dt * var_old + varint_old
        else:
            varintout.axpby(1.0, 0.0, varint_old)
            varintout.axpy(self.theta_ost * dt, var)
            varintout.axpy((1.0 - self.theta_ost) * dt, var_old)
    ...
    def update_varint_newmark_1st(self, var, var_old, varint_old, dt, varintout=None, uflform=True):
        if uflform:  # ufl form
            return self.gamma * dt * var + (1.0 - self.gamma) * dt * var_old + varint_old
        else:  # PETSc vector update
            varintout.axpby(1.0, 0.0, varint_old)
    ...
    def get_factor_deriv_dvar_ost(self, dt):
        return 1.0 / (self.theta_ost * dt)

    def get_factor_deriv_d2var_ost(self, dt):
        return 1.0 / (self.theta_ost * self.theta_ost * dt * dt)

    def get_factor_deriv_dvar_newmark_1st(self, dt):
        return 1.0 / (self.gamma * dt)
    ...
    def timecurves(self, cnum):
        if cnum == 0:
            return self.zero
        if cnum == 1:
            return self.time_curves.tc1
        if cnum == 2:
            return self.time_curves.tc2
        if cnum == 3:
            return self.time_curves.tc3
        if cnum == 4:
            return self.time_curves.tc4
        if cnum == 5:
            return self.time_curves.tc5
        if cnum == 6:
            return self.time_curves.tc6
        if cnum == 7:
            return self.time_curves.tc7
        if cnum == 8:
            return self.time_curves.tc8
        if cnum == 9:
            return self.time_curves.tc9
        if cnum == 10:
            return self.time_curves.tc10
        if cnum == 11:
            return self.time_curves.tc11
        if cnum == 12:
            return self.time_curves.tc12
        if cnum == 13:
            return self.time_curves.tc13
        if cnum == 14:
            return self.time_curves.tc14
        if cnum == 15:
            return self.time_curves.tc15
        if cnum == 16:
            return self.time_curves.tc16
        if cnum == 17:
            return self.time_curves.tc17
        if cnum == 18:
            return self.time_curves.tc18
        if cnum == 19:
            return self.time_curves.tc19
        if cnum == 20:
            return self.time_curves.tc20


# Solid mechanics time integration class
class timeintegration_solid(timeintegration):
    def __init__(self, time_params, dt, Nmax, fem_params, time_curves=None, t_init=0.0, dim=3, comm=None):
        timeintegration.__init__(
            self, time_params, dt, Nmax, time_curves=time_curves, t_init=t_init, dim=dim, comm=comm
        )
    ...
    def compute_genalpha_params(self, rho_inf):  # cf. Chung and Hulbert (1993)
        ...
        beta = 0.25 * (1.0 - alpha_m + alpha_f) ** 2.0
...
class timeintegration_fluid(timeintegration):
    ...
    def update_timestep(self, v, v_old, a, a_old, p, p_old, internalvars, internalvars_old, uf=None, uf_old=None):
        ...
        try:
            ...
            for j in range(len(subvecs)):
                subvecs[j].ghostUpdate(addv=PETSc.InsertMode.INSERT, mode=PETSc.ScatterMode.FORWARD)
    ...
    def update_fields(self, v, v_old, a, a_old, uf=None, uf_old=None):
        # use update functions using vector arguments
        self.update_dvar(
            v.x.petsc_vec, v_old.x.petsc_vec, a_old.x.petsc_vec, self.dt, dvarout=a.x.petsc_vec, uflform=False
        )

        if uf_old is not None:
            # use update functions using vector arguments
            self.update_varint(
                v.x.petsc_vec, v_old.x.petsc_vec, uf_old.x.petsc_vec, self.dt, varintout=uf.x.petsc_vec, uflform=False
            )
...
class timeintegration_ale(timeintegration_fluid):
    ...
    def update_fields(self, d, d_old, w, w_old):
        # use update functions using vector arguments
        self.update_dvar(
            d.x.petsc_vec, d_old.x.petsc_vec, w_old.x.petsc_vec, self.dt, dvarout=w.x.petsc_vec, uflform=False
        )
...
class timeintegration_flow0d(timeintegration):
    ...
    def __init__(self, time_params, dt, Nmax, time_curves=None, t_init=0.0, comm=None):
