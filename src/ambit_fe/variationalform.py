    ...
    def deltaW_ext_membrane(
        ...
        if active is not None:
            ...
            if params["active_stress"]["dir"] == "cl":
                ...
                omega, iota, gamma = (
                    params["active_stress"]["omega"],
                    params["active_stress"]["iota"],
                    params["active_stress"]["gamma"],
                )
                S_act = (
                    w_act
                    * tau
        ...
        if model == "membrane":
            ...
            raise RuntimeError(
                "Model 'membrane_fmod' seems incompatible and gives erroneous results. To be investigated..."
            )
            ...
            Umod = (
                ufl.sqrt(evalCmod[0]) * EprojCmod[0]
                + ufl.sqrt(evalCmod[1]) * EprojCmod[1]
                + ufl.sqrt(evalCmod[2]) * EprojCmod[2]
            )
            Fmod = R * Umod
        else:
        ...
        Ic = ufl.tr(Cmod)
        IIc = 0.5 * (ufl.tr(Cmod) ** 2.0 - ufl.tr(Cmod * Cmod))
        ...
        Psi_v = (eta / 8.0) * ufl.tr(Cmoddot_ * Cmoddot_)
        ...
        p = 2.0 * (dPsi_dIc / (IIIplane) - IIIplane * dPsi_dIIc)
        # balance viscous normal stresses
        p -= (eta / 2.0) * (IIIplanedot / (IIIplane**3.0))
