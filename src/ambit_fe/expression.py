...
class template_vector:
    ...
    def evaluate(self, x):
        if self.dim == 3:
            return (np.full(x.shape[1], self.val_x), np.full(x.shape[1], self.val_y), np.full(x.shape[1], self.val_z))
        elif self.dim == 2:
            return (np.full(x.shape[1], self.val_x), np.full(x.shape[1], self.val_y))
        else:
