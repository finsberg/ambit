...
class SolidmechanicsFlow0DMultiscaleGrowthRemodelingProblem(problem_base):
    def __init__(
        ...
        try:
            self.write_checkpoints = multiscale_params["write_checkpoints"]
        except:
            self.write_checkpoints = False

        try:
            self.restart_cycle = multiscale_params["restart_cycle"]
        except:
            self.restart_cycle = 0

        try:
            self.restart_from_small = multiscale_params["restart_from_small"]
        except:
            self.restart_from_small = False
...
class SolidmechanicsFlow0DMultiscaleGrowthRemodelingSolver(solver_base):
    ...
    def set_state_small(self):
        ...
        u_delta = PETSc.Vec().createMPI(
            (self.pb.pblarge.u.x.petsc_vec.getLocalSize(), self.pb.pblarge.u.x.petsc_vec.getSize()),
        ...
        if self.pb.pbsmall.pbs.incompressible_2field:
            p_delta = PETSc.Vec().createMPI(
                (self.pb.pblarge.p.x.petsc_vec.getLocalSize(), self.pb.pblarge.p.x.petsc_vec.getSize()),
    ...
    def set_state_large(self, N):
        ...
        self.pb.pbsmall.pb0.cardvasc0D.set_pressure_fem(
            self.pb.pbsmall.pb0.s_set, self.pb.pbsmall.pb0.cardvasc0D.v_ids, self.pb.pbsmall.pr0D, self.pb.neumann_funcs
        )
