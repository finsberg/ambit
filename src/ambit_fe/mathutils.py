...
def spectral_decomposition_3x3(A):
    ...
    j = ufl.tr(B * B) / 2.0  # == -I2(B) for trace-free B, j < 0 indicates A has complex eigenvalues
    b = ufl.tr(B * B * B) / 3.0  # == I3(B) for trace-free B
    ...
    p = (2.0 / ufl.sqrt(3.0)) * ufl.sqrt(j + eps**2.0)
    r = 4.0 * b / (p**3.0)
    r = ufl.max_value(ufl.min_value(r, 1.0 - eps), -1.0 + eps)
    phi = ufl.acos(r) / 3.0
    # sorted eigenvalues: lambda1 >= lambda2 >= lambda3
    lambda1 = q + p * ufl.cos(phi)  # high
    lambda2 = q + p * ufl.cos(phi + (4.0 / 3.0) * ufl.pi)  # middle
    lambda3 = q + p * ufl.cos(phi + (2.0 / 3.0) * ufl.pi)  # low
