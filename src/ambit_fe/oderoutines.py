...
class ode:
    ...
    def set_prescribed_variables_stiffness(self, K, index_prescribed):
        ...
        K[index_prescribed, index_prescribed] = 1.0
        for j in range(self.numdof):
            if j != index_prescribed:
                K[index_prescribed, j] = 0.0
    ...
    def set_output_state(self, var, var_old, var_out, theta, midpoint=True):
        ...
        if midpoint:
            var_out[vs:ve] = theta * var[vs:ve] + (1.0 - theta) * var_old[vs:ve]
        else:
    ...
    def set_solve_arrays(self):
        ...
        self.dK_, self.K_ = (
            [[0] * self.numdof for _ in range(self.numdof)],
            [[0] * self.numdof for _ in range(self.numdof)],
        )
        self.dK__, self.K__ = (
            [[0] * self.numdof for _ in range(self.numdof)],
    ...
    def write_initial(self, path, nm, varTc_old, varTc, auxTc_old, auxTc):
        if self.ode_parallel:
            varTc_old_arr, varTc_arr = allgather_vec(varTc_old, self.comm), allgather_vec(varTc, self.comm)
        else:
            varTc_old_arr, varTc_arr = varTc_old.array, varTc.array
