...
class ModelOrderReduction:
    def __init__(self, pb):
        ...
        try:
            self.modes_from_files = self.params["modes_from_files"]
        except:
            self.modes_from_files = False
        ...
        if not self.modes_from_files:
            ...
            try:
                self.snapshotincr = self.params["snapshotincr"]
            except:
                self.snapshotincr = 1

            try:
                self.snapshotoffset = self.params["snapshotoffset"]
            except:
                self.snapshotoffset = 0

            try:
                self.print_eigenproblem = self.params["print_eigenproblem"]
            except:
                self.print_eigenproblem = False

            try:
                self.eigenvalue_cutoff = self.params["eigenvalue_cutoff"]
            except:
                self.eigenvalue_cutoff = 0.0

            try:
                self.pod_only = self.params["pod_only"]
            except:
                self.pod_only = False
        else:
        ...
        try:
            self.numredbasisvec = self.params["numredbasisvec"]
        except:
            self.numredbasisvec = self.numsnapshots

        try:
            self.orthogonalize_rom_basis = self.params["orthogonalize_rom_basis"]
        except:
            self.orthogonalize_rom_basis = False

        try:
            self.surface_rom = self.params["surface_rom"]
        except:
            self.surface_rom = []
        ...
        try:
            ...
            self.filetype = "id_val"

        try:
            self.write_pod_modes = self.params["write_pod_modes"]
        except:
            self.write_pod_modes = False
        ...
        try:
            self.regularizations = self.params["regularizations"]
        except:
            self.regularizations = []

        try:
            self.regularizations_integ = self.params["regularizations_integ"]
        except:
            self.regularizations_integ = []

        try:
            self.regularizations_deriv = self.params["regularizations_deriv"]
        except:
            self.regularizations_deriv = []
        ...
        try:
            self.partitions = self.params["partitions"]
        except:
            self.partitions = []

        try:
            self.exclude_from_snap = self.params["exclude_from_snap"]
        except:
        ...
        try:
            self.numredbasisvec_partition = self.params["numredbasisvec_partition"]
        except:
            self.numredbasisvec_partition = [self.numredbasisvec] * self.num_partitions
        ...
        self.S_d = PETSc.Mat().createDense(
            size=((self.locmatsize_u, self.matsize_u), (self.num_hdms * self.numsnapshots)),
            bsize=None,
            array=None,
            comm=self.pb.comm,
        )
    ...
    def prepare_rob(self):
        ...
        if bool(self.exclude_from_snap):
            self.excl_set = set(
                gather_surface_dof_indices(self.pb.io, self.Vspace, self.exclude_from_snap, self.pb.comm)
            )
        ...
        for h in range(self.num_hdms):
            for i in range(self.numredbasisvec_true):
                ...
                self.pb.io.readfunction(
                    field, self.modes_from_files[h].replace("*", str(i + 1)), filetype=self.filetype
                )
    ...
    def build_reduced_basis(self):
        ...
        self.V = PETSc.Mat().createAIJ(
            size=((self.locmatsize_u, self.matsize_u), (PETSc.DECIDE, sum(self.numredbasisvec_partition))),
            bsize=None,
    ...
    def build_reduced_surface_basis(self):
        ...
        # create aij matrix - important to specify an approximation for nnz (number of non-zeros per row) for efficient value setting
        self.V = PETSc.Mat().createAIJ(
            size=((self.locmatsize_u, self.matsize_u), (PETSc.DECIDE, sum(self.numredbasisvec_partition) + ndof_bulk)),
            bsize=None,
            nnz=(sum(self.numredbasisvec_partition) + 1),
            csr=None,
            comm=self.pb.comm,
