...
class sol_utils:
    ...
    def catch_solver_errors(self, resnorm, incnorm=0, maxval=1e16, linconv=1):
        ...
        if np.isnan(resnorm):
            utilities.print_status("NaN encountered. Reset Newton and perform PTC adaption.", self.solver.comm)
        ...
        if header:
            if numres == 1:
                if not sub:
                    ...
                    utilities.print_status(
                        "       ****************** 0D model solve ******************", self.solver.comm
                    )
                    utilities.print_status(
                ...
                utilities.print_status(
                    ...
                    ).format(" ", "it |", eq1, "| ", eq2, "| ", eq3, "| ", eq4, "| ", "timings"),
                    self.solver.comm,
                )
                utilities.print_status(
                    (
                ...
                utilities.print_status(
                    ...
                    ).format(" ", "it |", eq1, "| ", eq2, "| ", eq3, "| ", eq4, "| ", eq5, "| ", "timings"),
                    self.solver.comm,
                )
                utilities.print_status(
                    (
                ...
                utilities.print_status(
                    ...
                    ).format(" ", "it |", eq1, "| ", eq2, "| ", eq3, "| ", eq4, "| ", eq5, "| ", eq6, "| ", "timings"),
                    self.solver.comm,
                )
                utilities.print_status(
                    (
            ...
            if numres == 1:
                if not sub:
                    utilities.print_status(
                        ...
                        ).format(" ", it, "| ", resnorms["res1"], " ", incnorms["inc1"], "  |  ", te, " ", ts),
                        self.solver.comm,
                    )
                else:
                    utilities.print_status(
        ...
        if it % self.solver.print_liniter_every == 0:
            ...
            utilities.print_status(
                ("{:<" + str(self.solver.indlen) + "s}{:<17s}{:<4d}{:<21s}{:<4e}").format(
                    " ", "        lin. it.: ", it, "     " + self.solver.res_lin_monitor + ". res. norm:", resnorm
                ),
                self.solver.comm,
            )
