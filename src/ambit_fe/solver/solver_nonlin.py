...
class solver_nonlinear:
    ...
    def set_solver_params(self, solver_params):
        try:
            self.maxiter = solver_params["maxiter"]
        except:
            self.maxiter = 25

        try:
            self.divcont = solver_params["divergence_continue"]
        except:
            self.divcont = None

        try:
            self.PTC = solver_params["ptc"]
        except:
            self.PTC = False

        try:
            self.k_PTC_initial = solver_params["k_ptc_initial"]
        except:
            self.k_PTC_initial = 0.1

        try:
            self.PTC_randadapt_range = solver_params["ptc_randadapt_range"]
        except:
            self.PTC_randadapt_range = [0.85, 1.35]

        try:
            self.maxresval = solver_params["catch_max_res_value"]
        except:
            self.maxresval = 1e16

        try:
            self.direct_solver = solver_params["direct_solver"]
        except:
            self.direct_solver = "mumps"

        try:
            self.iterative_solver = solver_params["iterative_solver"]
        except:
            self.iterative_solver = "gmres"

        try:
            precond_fields = solver_params["precond_fields"]
        except:
            precond_fields = [[]]
        ...
        try:
            self.fieldsplit_type = solver_params["fieldsplit_type"]
        except:
            self.fieldsplit_type = "jacobi"

        try:
            block_precond = solver_params["block_precond"]
        except:
        ...
        try:
            petsc_options_ksp = solver_params["petsc_options_ksp"]
        except:
            petsc_options_ksp = None
        ...
        try:
            self.tol_lin_rel = solver_params["tol_lin_rel"]
        except:
            self.tol_lin_rel = 1e-5

        try:
            self.tol_lin_abs = solver_params["tol_lin_abs"]
        except:
            self.tol_lin_abs = 1e-50

        try:
            self.res_lin_monitor = solver_params["res_lin_monitor"]
        except:
            self.res_lin_monitor = "rel"

        try:
            self.maxliniter = solver_params["max_liniter"]
        except:
            self.maxliniter = 1000

        try:
            self.lin_norm_type = solver_params["lin_norm_type"]
        except:
            self.lin_norm_type = "unpreconditioned"
        ...
        try:
            self.print_liniter_every = solver_params["print_liniter_every"]
        except:
            self.print_liniter_every = 1

        try:
            self.iset_options = solver_params["indexset_options"]
        except:
            self.iset_options = {}
        ...
        try:
            self.print_local_iter = solver_params["print_local_iter"]
        except:
            self.print_local_iter = False

        try:
            self.rebuild_prec_every_it = solver_params["rebuild_prec_every_it"]
        except:
            self.rebuild_prec_every_it = 1

        try:
            self.tol_res_local = solver_params["tol_res_local"]
        except:
            self.tol_res_local = 1e-10

        try:
            self.tol_inc_local = solver_params["tol_inc_local"]
        except:
            self.tol_inc_local = 1e-10
            ...
            if self.solvetype[npr] == "direct":
                ...
                if self.nfields[npr] > 1:
                    ...
                    utilities.print_status(
                        "Creating merged solver residual and Jacobian data structures...", self.comm, e=" "
                    )
                ...
                if self.nfields[npr] > 1:
                    ...
                    if self.merge_prec_mat:
                        ts = time.time()
                        utilities.print_status(
                            "Creating merged solver preconditioner data structures...", self.comm, e=" "
                        )
                ...
                if self.nfields[npr] > 1:
                    ...
                    if self.block_precond[npr] == "fieldsplit":
                        ...
                        if nsets == 2:
                            self.ksp[npr].getPC().setFieldSplitIS(("f1", self.iset[npr][0]), ("f2", self.iset[npr][1]))
                        elif nsets == 3:
                            self.ksp[npr].getPC().setFieldSplitIS(
                                ("f1", self.iset[npr][0]), ("f2", self.iset[npr][1]), ("f3", self.iset[npr][2])
                            )
                        elif nsets == 4:
                            self.ksp[npr].getPC().setFieldSplitIS(
                                ("f1", self.iset[npr][0]),
                        ...
                        for n in range(nsets):
                            if self.precond_fields[npr][n]["prec"] == "amg":
                                try:
                                    solvetype = self.precond_fields[npr][n]["solve"]
                                except:
                                    solvetype = "preonly"
                                ksp_fields[n].setType(solvetype)
                                try:
                                    amgtype = self.precond_fields[npr][n]["amgtype"]
                                except:
                                    amgtype = "hypre"
                        ...
                        bj = preconditioner.schur_2x2(
                            self.iset[npr], self.precond_fields[npr], self.pb[npr].io, self.solver_params, self.comm
                        )
                        self.ksp[npr].getPC().setPythonContext(bj)

                    elif self.block_precond[npr] == "schur2x2simple":
                        self.ksp[npr].getPC().setType(PETSc.PC.Type.PYTHON)
                        bj = preconditioner.schur_2x2simple(
                            self.iset[npr], self.precond_fields[npr], self.pb[npr].io, self.solver_params, self.comm
                        )
                        self.ksp[npr].getPC().setPythonContext(bj)
                        ...
                        self.ksp[npr].getPC().setType(PETSc.PC.Type.PYTHON)
                        bj = preconditioner.schur_3x3simple(
                            self.iset[npr], self.precond_fields[npr], self.pb[npr].io, self.solver_params, self.comm
                        )
                        self.ksp[npr].getPC().setPythonContext(bj)
                        ...
                        self.ksp[npr].getPC().setType(PETSc.PC.Type.PYTHON)
                        bj = preconditioner.bgs_schur_4x4(
                            self.iset[npr], self.precond_fields[npr], self.pb[npr].io, self.solver_params, self.comm
                        )
                        self.ksp[npr].getPC().setPythonContext(bj)
                        ...
                        self.ksp[npr].getPC().setType(PETSc.PC.Type.PYTHON)
                        bj = preconditioner.bgs_schur_4x4simple(
                            self.iset[npr], self.precond_fields[npr], self.pb[npr].io, self.solver_params, self.comm
                        )
                        self.ksp[npr].getPC().setPythonContext(bj)
                        ...
                        self.ksp[npr].getPC().setType(PETSc.PC.Type.PYTHON)
                        bj = preconditioner.bgssym_schur_4x4(
                            self.iset[npr], self.precond_fields[npr], self.pb[npr].io, self.solver_params, self.comm
                        )
                        self.ksp[npr].getPC().setPythonContext(bj)
                        ...
                        self.ksp[npr].getPC().setType(PETSc.PC.Type.PYTHON)
                        bj = preconditioner.bgssym_schur_4x4simple(
                            self.iset[npr], self.precond_fields[npr], self.pb[npr].io, self.solver_params, self.comm
                        )
                        self.ksp[npr].getPC().setPythonContext(bj)
                ...
                self.ksp[npr].setTolerances(
                    rtol=self.tol_lin_rel, atol=self.tol_lin_abs, divtol=None, max_it=self.maxliniter
                )
        ...
        while it < self.maxiter and counter_adapt < max_adapt:
            ...
            for npr in range(self.nprob):
                ...
                if self.nfields[npr] > 1:
                    ...
                    if self.solvetype[npr] == "direct":
                        ...
                        if self.pb[npr].io.print_enhanced_info:
                            utilities.print_status(
                                " " * self.indlen_[npr] + "      === MAT merge, te = %.4f s" % (tme), self.comm
                            )
                    ...
                    # for nested iterative solver
                        ...
                        self.solutils.print_linear_iter_last(
                            self.ksp[npr].getIterationNumber(), self.ksp[npr].getResidualNorm(), linconv
                        )
                ...
                self.solutils.print_nonlinear_iter(
                    it, resnorms=self.resnorms[npr], incnorms=self.incnorms[npr], ts=ts, te=te, ptype=self.ptype[npr]
                )

                # get converged state of each problem
                converged.append(
                    self.solutils.check_converged(
                        self.resnorms[npr], self.incnorms[npr], self.tolerances[npr], ptype=self.ptype[npr]
                    )
                )
    ...
    def residual_problem_actions(self, t, npr, localdata):
        ...
        if self.pb[npr].io.print_enhanced_info:
            utilities.print_status(
                " " * self.indlen_[npr] + "      === Residual assemble, t = %.4f s" % (tee), self.comm
            )
    ...
    def jacobian_problem_actions(self, t, npr, k_PTC):
        ...
        if self.pb[npr].io.print_enhanced_info:
            utilities.print_status(
                " " * self.indlen_[npr] + "      === Jacobian assemble, t = %.4f s" % (tee), self.comm
            )
        ...
        while it_local < maxiter_local:
            for i in range(num_loc_res):
                ...
                increment_proj = project(
                    increment_forms[i], functionspaces[i], self.pb[0].dx, domids=self.pb[0].domain_ids, comm=self.comm
                )
            ...
            for i in range(num_loc_res):
                ...
                residual_proj = project(
                    residual_forms[i], functionspaces[i], self.pb[0].dx, domids=self.pb[0].domain_ids, comm=self.comm
                )
...
class solver_nonlinear_ode(solver_nonlinear):
    ...
    def newton(self, t, print_iter=True, sub=False):
        ...
        while it < self.maxiter:
            ...
            if print_iter:
                self.solutils.print_nonlinear_iter(
                    it, resnorms=self.resnorms, incnorms=self.incnorms, ts=ts, te=te, sub=sub, ptype=self.ptype
                )
