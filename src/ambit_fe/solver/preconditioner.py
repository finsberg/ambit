...
class block_precond:
    def __init__(self, iset, precond_fields, io, solparams, comm=None):
        ...
        try:
            schur_block_scaling = self.solparams["schur_block_scaling"]
        except:
            schur_block_scaling = [{"type": "diag", "val": 1.0}] * 2
    ...
    def create(self, pc):
        ...
        for n in range(self.nfields):
            if self.precond_fields[n]["prec"] == "amg":
                try:
                    solvetype = self.precond_fields[n]["solve"]
                except:
                    solvetype = "preonly"
                ...
                if solvetype == "gmres" or solvetype == "fgmres":
                    try:
                        maxiter = self.precond_fields[n]["maxiter"]
                    except:
                        maxiter = 1000
                    try:
                        tolrel = self.precond_fields[n]["tolrel"]
                    except:
                        tolrel = 1e-5
                    try:
                        tolabs = self.precond_fields[n]["tolabs"]
                    except:
                        tolabs = 1e-50
                    self.ksp_fields[n].setTolerances(rtol=tolrel, atol=tolabs, divtol=None, max_it=maxiter)
                try:
                    amgtype = self.precond_fields[n]["amgtype"]
                except:
                    amgtype = "hypre"
                ...
                if solvetype == "python":
                    try:
                        niter = self.precond_fields[n]["stat_iter"]
                    except:
                        niter = 1
...
class schur_3x3(block_precond):
    ...
    def init_mat_vec(self, pc):
        ...
        self.Smod.axpy(-1.0, self.B_Adinv_Bt)
        self.Umod.axpy(-1.0, self.D_Adinv_Bt)
        self.Tmod.axpy(-1.0, self.B_Adinv_Dt)
