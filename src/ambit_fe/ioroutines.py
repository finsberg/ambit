...
class IO:
    def __init__(self, io_params, fem_params, entity_maps, comm):
        ...
        try:
            self.output_path_pre = io_params["output_path_pre"]
        except:
            self.output_path_pre = self.output_path

        self.mesh_domain = io_params["mesh_domain"]
        try:
            self.mesh_boundary = io_params["mesh_boundary"]
        except:
            self.mesh_boundary = None
        try:
            self.mesh_edge = io_params["mesh_edge"]
        except:
            self.mesh_edge = None
        try:
            self.mesh_point = io_params["mesh_point"]
        except:
        ...
        try:
            self.fiber_data = io_params["fiber_data"]
        except:
            self.fiber_data = []
        ...
        try:
            self.meshfile_format = io_params["meshfile_format"]
        except:
            self.meshfile_format = "XDMF"

        try:
            self.meshfile_type = io_params["meshfile_type"]
        except:
            self.meshfile_type = "ASCII"

        try:
            self.mesh_dim = io_params["mesh_dim"]  # actually only needed/used for gmsh read-in
        except:
            self.mesh_dim = 3

        try:
            self.gridname_domain = io_params["gridname_domain"]
        except:
            self.gridname_domain = "Grid"

        try:
            self.gridname_boundary = io_params["gridname_boundary"]
        except:
            self.gridname_boundary = "Grid"

        try:
            self.duplicate_mesh_domains = io_params["duplicate_mesh_domains"]
        except:
            self.duplicate_mesh_domains = []

        try:
            self.restart_io_type = io_params["restart_io_type"]
        except:
            self.restart_io_type = "petscvector"

        try:
            self.indicate_results_by = io_params["indicate_results_by"]
        except:
            self.indicate_results_by = "time"

        try:
            self.print_enhanced_info = io_params["print_enhanced_info"]
        except:
            self.print_enhanced_info = False
        ...
        if self.mesh.topology.dim == 3:
            ...
            if self.mesh_edge is not None:
                ...
                self.mesh_e = mesh.create_submesh(
                    self.mesh, self.mesh.topology.dim - 2, self.mt_b2.indices[self.mt_b2.values == 1]
                )[0:2]
    ...
    def create_integration_measures(self, msh, mt_data):
        ...
        if mt_data[0] is not None:
            dx = ufl.Measure(
                "dx", domain=msh, subdomain_data=mt_data[0], metadata={"quadrature_degree": self.quad_degree}
            )
        else:
            dx_ = ufl.Measure("dx", domain=msh, metadata={"quadrature_degree": self.quad_degree})
            dx = lambda a: dx_  # so that we can call dx(1) even without domain meshtags

        if mt_data[1] is not None:
            ds = ufl.Measure(
                "ds", domain=msh, subdomain_data=mt_data[1], metadata={"quadrature_degree": self.quad_degree}
            )
        else:
            ds = None
        if mt_data[2] is not None:
            de = ufl.Measure(
                "ds", domain=msh, subdomain_data=mt_data[2], metadata={"quadrature_degree": self.quad_degree}
            )
        else:
            de = None
        if mt_data[1] is not None:
    ...
    def readfunction(self, f, datafile, normalize=False, filetype="id_val"):
        ...
        if filetype == "id_val":
            ...
            data_ = hf["/Mesh/mesh/uvc_longitudinal"]  # TODO:::
            if data_.ndim > 1:
                dim_ = data_.shape[1]
    ...
    def readin_fibers(self, fibarray, V_fib, dx_, domids, order_disp):
        ...
        try:
            self.order_fib_input = self.io_params["order_fib_input"]
        except:
                ...
                for res in pb.results_to_write:
                    if res == "displacement":
                        ...
                        cauchystress_membrane_principal_out = fem.Function(
                            pb.V_out_vector, name=self.cauchystress_membrane_principal.name
                        )
                        ...
                        for n in range(pb.num_domains):
                            if pb.mat_growth[n]:
                                stressfuncs.append(
                                    tr(pb.ma[n].M_e(pb.u, pb.p, pb.vel, pb.ki.C(pb.u), ivar=pb.internalvars))
                                )
                            else:
                        ...
                        for n in range(pb.num_domains):
                            sefuncs.append(
                                pb.ma[n].S(pb.u, pb.p, pb.vel, ivar=pb.internalvars, returnquantity="strainenergy")
                            )
                        se = project(
                            sefuncs,
                            pb.Vd_scalar,
                        ...
                        for n in range(pb.num_domains):
                            pwfuncs.append(
                                ufl.inner(
                                    pb.ma[n].S(pb.u, pb.p, pb.vel, ivar=pb.internalvars), pb.ki.Edot(pb.u, pb.vel)
                                )
                            )
                        pw = project(
                            pwfuncs,
                        ...
                        if bool(self.duplicate_mesh_domains):
                            for m, mp in enumerate(self.duplicate_mesh_domains):
                                V_out_scalar_sub = fem.functionspace(
                                    self.submshes_emap[m + 1][0], ("Lagrange", pb.mesh_degree)
                                )
...
class IO_fsi(IO_solid, IO_fluid, IO_ale):
    ...
    def create_submeshes(self):
        ...
        self.msh_emap_solid = mesh.create_submesh(
            self.mesh, self.mesh.topology.dim, self.mt_d.indices[np.isin(self.mt_d.values, self.dom_solid)]
        )[0:2]
        self.msh_emap_fluid = mesh.create_submesh(
            self.mesh, self.mesh.topology.dim, self.mt_d.indices[np.isin(self.mt_d.values, self.dom_fluid)]
        )[0:2]
        ...
        self.mt_d_solid = meshutils.meshtags_parent_to_child(
            self.mt_d, self.msh_emap_solid[0], self.msh_emap_solid[1], self.mesh, "domain"
        )
        self.mt_d_fluid = meshutils.meshtags_parent_to_child(
            self.mt_d, self.msh_emap_fluid[0], self.msh_emap_fluid[1], self.mesh, "domain"
        )

        self.mt_b1_solid = meshutils.meshtags_parent_to_child(
            self.mt_b1, self.msh_emap_solid[0], self.msh_emap_solid[1], self.mesh, "boundary"
        )
        self.mt_b1_fluid = meshutils.meshtags_parent_to_child(
            self.mt_b1, self.msh_emap_fluid[0], self.msh_emap_fluid[1], self.mesh, "boundary"
        )

        self.msh_emap_lm = mesh.create_submesh(
            self.mesh, self.mesh.topology.dim - 1, self.mt_b1.indices[np.isin(self.mt_b1.values, self.surf_interf)]
        )[0:2]
    ...
    def create_integration_measures(self, msh):
        ...
        if self.mt_d is not None:
            self.dx = ufl.Measure(
                "dx", domain=msh, subdomain_data=self.mt_d, metadata={"quadrature_degree": self.quad_degree}
            )
        else:
        ...
        for mt in meshtags:
            ...
            meshutils.get_integration_entities(
                msh, other_indices, self.mesh.topology.dim - 1, other_integration_entities
            )
            # append
        ...
        self.ds = ufl.Measure(
            "ds", domain=msh, subdomain_data=integration_entities, metadata={"quadrature_degree": self.quad_degree}
        )
        self.de = None
        self.dS = ufl.Measure(
            "dS", domain=msh, subdomain_data=integration_entities, metadata={"quadrature_degree": self.quad_degree}
