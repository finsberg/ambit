...
class FluidmechanicsAleProblem(problem_base):
    def __init__(
        ...
        try:
            self.coupling_fluid_ale = self.coupling_params["coupling_fluid_ale"]
        except:
            self.coupling_fluid_ale = {}

        try:
            self.coupling_ale_fluid = self.coupling_params["coupling_ale_fluid"]
        except:
            self.coupling_ale_fluid = {}

        try:
            self.coupling_strategy = self.coupling_params["coupling_strategy"]
        except:
    ...
    def set_variational_forms(self):
            ...
            for j in range(len(self.coupling_fluid_ale)):
                ...
                if self.coupling_fluid_ale[j]["type"] == "strong_dirichlet":
                    ...
                    self.fdofs = PETSc.IS().createBlock(
                        self.pba.V_d.dofmap.index_map_bs, fnode_indices_all, comm=self.comm
                    )

                elif self.coupling_fluid_ale[j]["type"] == "weak_dirichlet":
                    beta = self.coupling_fluid_ale[j]["beta"]
                    try:
                        hscale = self.coupling_fluid_ale[j]["hscale"]
                    except:
                        hscale = False
        ...
        if bool(self.coupling_ale_fluid):
            dbcs_coup_ale_fluid, work_robin_ale_fluid, work_robin_ale_fluid_old, work_robin_ale_fluid_mid = (
                [],
                ufl.as_ufl(0),
                ufl.as_ufl(0),
                ufl.as_ufl(0),
                    ...
                    for i in range(len(ids_ale_fluid)):
                        if self.coupling_ale_fluid[j]["type"] == "robin":
                            ...
                            work_robin_ale_fluid += self.pbf.vf.deltaW_int_robin_cur(
                                self.pbf.v, self.pba.wel, beta, db_, Fale=self.pba.ki.F(self.pba.d)
                            )  # here, wel as form is used!
                            work_robin_ale_fluid_old += self.pbf.vf.deltaW_int_robin_cur(
                                self.pbf.v_old, self.pba.w_old, beta, db_, Fale=self.pba.ki.F(self.pba.d_old)
                            )
        ...
        if self.coupling_strategy == "monolithic":
            ...
            if self.have_weak_dirichlet_fluid_ale:
                ...
                self.Diag_ale = PETSc.Mat().createAIJ(
                    self.pba.K_dd.getSizes(), bsize=None, nnz=(1, 1), csr=None, comm=self.comm
                )
                self.Diag_ale.setUp()
