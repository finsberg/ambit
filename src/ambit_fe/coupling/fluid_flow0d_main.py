...
class FluidmechanicsFlow0DProblem(problem_base):
    def __init__(
        ...
        try:
            self.surface_p_ids = self.coupling_params["surface_p_ids"]
        except:
            self.surface_p_ids = self.surface_vq_ids

        self.num_coupling_surf = len(self.surface_vq_ids)

        try:
            self.cq_factor = self.coupling_params["cq_factor"]
        except:
            self.cq_factor = [1.0] * self.num_coupling_surf

        try:
            self.eps_fd = self.coupling_params["eps_fd"]
        except:
            self.eps_fd = 1.0e-5

        try:
            self.print_subiter = self.coupling_params["print_subiter"]
        except:
            self.print_subiter = False

        try:
            self.restart_periodicref = self.coupling_params["restart_periodicref"]
        except:
            self.restart_periodicref = 0

        try:
            self.Nmax_periodicref = self.coupling_params["Nmax_periodicref"]
        except:
            self.Nmax_periodicref = 10

        try:
            self.condense_0d_model = self.coupling_params["condense_0d_model"]
        except:
            self.condense_0d_model = "no"
        ...
        self.pb0 = Flow0DProblem(
            pbase, io_params, time_params_flow0d, model_params_flow0d, time_curves, coupling_params
        )
    ...
    def set_variational_forms(self):
        ...
        self.power_coupling, self.power_coupling_old, self.power_coupling_mid = (
            ufl.as_ufl(0),
            ufl.as_ufl(0),
            ufl.as_ufl(0),
        )
        ...
        for n in range(self.num_coupling_surf):
            ...
            (
                self.coupfuncs.append(fem.Function(self.pbf.Vd_scalar)),
                self.coupfuncs_old.append(fem.Function(self.pbf.Vd_scalar)),
            )
            ...
            self.coupfuncs_mid.append(
                self.pbf.timefac * self.coupfuncs[-1] + (1.0 - self.pbf.timefac) * self.coupfuncs_old[-1]
            )
            ...
            for i in range(len(self.surface_vq_ids[n])):
                ...
                cq_old_ += self.pbf.vf.flux(
                    self.pbf.v_old, ds_vq, w=self.pbf.alevar["w_old"], F=self.pbf.alevar["Fale_old"]
                )
            ...
            for i in range(len(self.surface_p_ids[n])):
                ...
                df_ += self.pbf.timefac * self.pbf.vf.flux(
                    self.pbf.var_v, ds_p, w=ufl.constantvalue.zero(self.pbf.ki.dim), F=self.pbf.alevar["Fale"]
                )
                df_mid_ += self.pbf.timefac * self.pbf.vf.flux(
                    self.pbf.var_v, ds_p, w=ufl.constantvalue.zero(self.pbf.ki.dim), F=self.pbf.alevar["Fale_mid"]
                )

                # add to fluid rhs contributions
                self.power_coupling += self.pbf.vf.deltaW_ext_neumann_normal_cur(
                    self.coupfuncs[-1], ds_p, F=self.pbf.alevar["Fale"]
                )
                self.power_coupling_old += self.pbf.vf.deltaW_ext_neumann_normal_cur(
                    self.coupfuncs_old[-1], ds_p, F=self.pbf.alevar["Fale_old"]
                )
                self.power_coupling_mid += self.pbf.vf.deltaW_ext_neumann_normal_cur(
                    self.coupfuncs_mid[-1], ds_p, F=self.pbf.alevar["Fale_mid"]
                )
    ...
    def set_problem_vector_matrix_structures_coupling(self):
        ...
        self.K_lm = PETSc.Mat().createAIJ(
            size=(self.num_coupling_surf, self.num_coupling_surf), bsize=None, nnz=None, csr=None, comm=self.comm
        )
        self.K_lm.setUp()
        ...
        for n in range(self.num_coupling_surf):
            ...
            nds_vq = np.array(
                self.pbf.V_v.dofmap.index_map.local_to_global(np.asarray(nds_vq_local, dtype=np.int32)), dtype=np.int32
            )
            ...
            nds_p_local = fem.locate_dofs_topological(
                self.pbf.V_v,
                self.pbf.io.mesh.topology.dim - 1,
                self.pbf.io.mt_b1.indices[np.isin(self.pbf.io.mt_b1.values, self.surface_p_ids[n])],
            )
            nds_p = np.array(
                self.pbf.V_v.dofmap.index_map.local_to_global(np.asarray(nds_p_local, dtype=np.int32)), dtype=np.int32
            )
        ...
        self.K_vs = PETSc.Mat().createAIJ(
            size=((locmatsize, matsize), (PETSc.DECIDE, self.num_coupling_surf)),
            bsize=None,
        ...
        self.K_sv = PETSc.Mat().createAIJ(
            size=((PETSc.DECIDE, self.num_coupling_surf), (locmatsize, matsize)),
            bsize=None,
            nnz=max(sze_sv),
            csr=None,
            comm=self.comm,
        ...
        if self.condense_0d:
            ...
            self.K_lm_inv = PETSc.Mat().createAIJ(
                size=(self.num_coupling_surf, self.num_coupling_surf),
                bsize=None,
                nnz=self.num_coupling_surf * self.num_coupling_surf,
                csr=None,
                comm=self.comm,
            )
            self.K_lm_inv.setUp()
            if self.condense_0d_model == "diag":
                self.K_lm_invmat = PETSc.Mat().createAIJ(
                    size=(self.num_coupling_surf, self.num_coupling_surf),
                    bsize=None,
                    nnz=self.num_coupling_surf,
                    csr=None,
            ...
            for i in range(len(self.col_ids)):
                ...
                self.K_vs.setValues(
                    self.dofs_coupling_p[i], self.col_ids[i], self.k_vs_subvec[i].array, addv=PETSc.InsertMode.INSERT
                )
            ...
            for i in range(len(self.row_ids)):
                ...
                self.K_sv.setValues(
                    self.row_ids[i], self.dofs_coupling_vq[i], self.k_sv_subvec[i].array, addv=PETSc.InsertMode.INSERT
                )
            ...
            self.K_lm_inv.setValuesBlocked(
                islm,
                islm,
                np.ones((self.num_coupling_surf, self.num_coupling_surf), dtype=np.int32),
    ...
    def assemble_stiffness_3d0d(self, t, LM_sq, subsolver=None, condensed_res_action=False):
        ...
        for i in range(len(self.col_ids)):
            with self.k_vs_vec[i].localForm() as r_local:
                r_local.set(0.0)
            fem.petsc.assemble_vector(
                self.k_vs_vec[i], self.dforce_form[i]
    ...
    def update_condensed_vars(self, del_x):
        ...
        self.r_lm_kcondens.axpby(-1.0, 0.0, self.r_lm)
        self.r_lm_kcondens.axpy(-1.0, self.ksv_v)
    ...
    def evaluate_initial(self):
        ...
        self.pb0.cardvasc0D.set_pressure_fem(
            self.LM_old, list(range(self.num_coupling_surf)), self.pr0D, self.coupfuncs_old
        )
        ...
        if bool(self.pb0.chamber_models):
            if (
                ...
                if self.pbase.restart_step == 0:
                    self.pb0.auxdata_old["p"] = copy.deepcopy(
                        self.pbf.pu_old_
                    )  # copy since we write restart and update auxdata_old differently
        ...
        if bool(self.pb0.chamber_models):
            for i, ch in enumerate(["lv", "rv", "la", "ra"]):
                if self.pb0.chamber_models[ch]["type"] == "0D_elast":
                    self.pb0.y[i] = self.pb0.ti.timecurves(self.pb0.chamber_models[ch]["activation_curve"])(
                        self.pbase.t_init
                    )
                if self.pb0.chamber_models[ch]["type"] == "0D_elast_prescr":
                    self.pb0.y[i] = self.pb0.ti.timecurves(self.pb0.chamber_models[ch]["elastance_curve"])(
                        self.pbase.t_init
                    )
                if self.pb0.chamber_models[ch]["type"] == "0D_prescr":
                    self.pb0.c.append(
                        self.pb0.ti.timecurves(self.pb0.chamber_models[ch]["prescribed_curve"])(self.pbase.t_init)
                    )
        ...
        if self.pbf.io.write_results_every > 0 and N % self.pbf.io.write_results_every == 0:
            ...
            if self.comm.rank == 0:
                for i in range(len(LM_sq)):
                    f = open(
                        self.pbase.output_path + "/results_" + self.pbase.simname + "_LM" + str(i + 1) + ".txt", mode
                    )
                    f.write("%.16E %.16E\n" % (t, LM_sq[i]))
                    f.close()
            del LM_sq
...
class FluidmechanicsFlow0DSolver(solver_base):
    ...
    def solve_initial_state(self):
        ...
        if (
            ...
            res_a, jac_aa = (
                fem.form(weakform_a, entity_maps=self.pb.pbf.io.entity_maps),
                fem.form(weakform_lin_aa, entity_maps=self.pb.pbf.io.entity_maps),
            )
