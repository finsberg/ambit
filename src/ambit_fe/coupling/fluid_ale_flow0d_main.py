...
class FluidmechanicsAleFlow0DProblem(FluidmechanicsAleProblem, problem_base):
    def __init__(
        ...
        try:
            self.coupling_fluid_ale = coupling_params_fluid_ale["coupling_fluid_ale"]
        except:
            self.coupling_fluid_ale = {}

        try:
            self.coupling_ale_fluid = coupling_params_fluid_ale["coupling_ale_fluid"]
        except:
            self.coupling_ale_fluid = {}

        try:
            self.coupling_strategy = coupling_params_fluid_ale["coupling_strategy"]
        except:
            self.coupling_strategy = "monolithic"
    ...
    def get_problem_var_list(self):
        if self.pbf0.pbf.num_dupl > 1:
            is_ghosted = [1, 2, 0, 1]
        else:
            is_ghosted = [1, 1, 0, 1]
        return [self.pbf.v.x.petsc_vec, self.pbf.p.x.petsc_vec, self.pbf0.LM, self.pba.d.x.petsc_vec], is_ghosted
        ...
        if self.coupling_strategy == "monolithic":
            ...
            for n in range(self.pbf0.num_coupling_surf):
                nds_vq_local = fem.locate_dofs_topological(
                    ...
                    self.pba.io.mt_b1.indices[np.isin(self.pba.io.mt_b1.values, self.pbf0.surface_vq_ids[n])],
                )
                nds_vq = np.array(
                ...
                self.dofs_coupling_vq[n] = PETSc.IS().createBlock(
                    self.pba.V_d.dofmap.index_map_bs, nds_vq, comm=self.comm
                )
            ...
            self.K_sd = PETSc.Mat().createAIJ(
                size=((PETSc.DECIDE, self.pbf0.num_coupling_surf), (locmatsize, matsize)),
                bsize=None,
                nnz=max(sze_sd),
                csr=None,
                comm=self.comm,
    ...
    def assemble_stiffness(self, t, subsolver=None):
        ...
        for i in range(len(self.pbf0.row_ids)):
            ...
            self.K_sd.setValues(
                self.pbf0.row_ids[i], self.dofs_coupling_vq[i], self.k_sd_subvec[i].array, addv=PETSc.InsertMode.INSERT
            )
...
class FluidmechanicsAleFlow0DSolver(solver_base):
    def initialize_nonlinear_solver(self):
        ...
        if self.pb.coupling_strategy == "monolithic":
            ...
            self.solnln = solver_nonlin.solver_nonlinear(
                [self.pb.pbf0, self.pb.pba], self.solver_params, subsolver=self.subsol, cp=self.pb
            )
        else:
    ...
    def evaluate_assemble_system_initial(self, subsolver=None):
        ...
        if self.pb.coupling_strategy == "monolithic":
            self.pb.assemble_residual(
                self.pb.pbase.t_init, subsolver=None
            )  # note: subsolver only passed to stiffness eval to get correct sparsity pattern)
            ...
            if self.pb.rom:
                ...
                if self.pb.pbf.pre:
                    ...
                    self.pb.pbf.rom.set_reduced_data_structures_matrix(
                        self.pb.pbf.K_list, self.pb.pbf.K_list_rom, self.pb.pbf.K_list_tmp
                    )
            ...
            if self.pb.pbf0.rom:
                ...
                self.pb.pbf0.rom.set_reduced_data_structures_matrix(
                    self.pb.pbf0.K_list, self.pb.pbf0.K_list_rom, self.pb.pbf0.K_list_tmp
                )
