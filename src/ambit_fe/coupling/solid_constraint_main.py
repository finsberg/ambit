...
class SolidmechanicsConstraintProblem(problem_base):
        ...
        for n in range(self.num_coupling_surf):
            ...
            for i in range(len(self.surface_vq_ids[n])):
                ...
                if self.coupling_params["constraint_physics"][n]["type"] == "volume":
                    cq_ += self.vq_scales[n][i] * self.pbs.vf.volume(
                        self.pbs.u, ds_vq, F=self.pbs.ki.F(self.pbs.u, ext=True)
                    )
                    cq_old_ += self.vq_scales[n][i] * self.pbs.vf.volume(
                        self.pbs.u_old, ds_vq, F=self.pbs.ki.F(self.pbs.u_old, ext=True)
                    )
                elif self.coupling_params["constraint_physics"][n]["type"] == "flux":
                    cq_ += self.vq_scales[n][i] * self.pbs.vf.flux(
                        self.pbs.vel, ds_vq, F=self.pbs.ki.F(self.pbs.u, ext=True)
                    )
                    cq_old_ += self.vq_scales[n][i] * self.pbs.vf.flux(
                        self.pbs.v_old, ds_vq, F=self.pbs.ki.F(self.pbs.u_old, ext=True)
                    )
                else:
            ...
            for i in range(len(self.surface_lm_ids[n])):
                if self.coupling_params["multiplier_physics"][n]["type"] == "pressure":
                    ...
                    df_ += self.pbs.timefac * self.pbs.vf.flux(
                        self.pbs.var_u, ds_p, F=self.pbs.ki.F(self.pbs.u, ext=True)
                    )
    ...
    def read_restart(self, sname, N):
        ...
        if N > 0:
            restart_data = np.loadtxt(
                self.pbs.io.output_path + "/checkpoint_" + sname + "_lm_" + str(N) + ".txt", ndmin=1
            )
    ...
    def evaluate_pre_solve(self, t, N, dt):
        ...
        for n in range(self.num_coupling_surf):
            self.constr_val[n] = self.pbs.ti.timecurves(
