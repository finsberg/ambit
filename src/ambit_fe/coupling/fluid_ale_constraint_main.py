...
class FluidmechanicsAleConstraintProblem(FluidmechanicsAleProblem, problem_base):
        ...
        if self.coupling_strategy == "monolithic":
            ...
            for n in range(self.pbfc.num_coupling_surf):
                nds_c_local = fem.locate_dofs_topological(
                    ...
                    self.pba.io.mt_b1.indices[np.isin(self.pba.io.mt_b1.values, self.pbfc.surface_vq_ids[n])],
                )
                nds_c = np.array(
            ...
            self.K_sd = PETSc.Mat().createAIJ(
                size=((PETSc.DECIDE, self.pbfc.num_coupling_surf), (locmatsize, matsize)),
                bsize=None,
                nnz=max(sze_sd),
                csr=None,
                comm=self.comm,
    ...
    def assemble_stiffness(self, t, subsolver=None):
        ...
        for i in range(len(self.pbfc.row_ids)):
            ...
            self.K_sd.setValues(
                self.pbfc.row_ids[i], self.dofs_coupling_vq[i], self.k_sd_subvec[i].array, addv=PETSc.InsertMode.INSERT
            )
    ...
    def solve_initial_state(self):
        ...
        if (
            ...
            weakform_a = (
                ...
                - self.pb.pbfc.power_coupling_old
            )
    ...
    def evaluate_assemble_system_initial(self, subsolver=None):
        ...
        if self.pb.coupling_strategy == "monolithic":
            ...
            if self.pb.pbfc.rom:
                ...
                self.pb.pbfc.rom.set_reduced_data_structures_matrix(
                    self.pb.pbfc.K_list, self.pb.pbfc.K_list_rom, self.pb.pbfc.K_list_tmp
                )
