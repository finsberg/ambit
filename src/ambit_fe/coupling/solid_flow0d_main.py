...
class SolidmechanicsFlow0DProblem(problem_base):
    def __init__(
        ...
        try:
            self.coupling_type = self.coupling_params["coupling_type"]
        except:
            self.coupling_type = "monolithic_direct"
        ...
        try:
            self.write_checkpoints_periodicref = self.coupling_params["write_checkpoints_periodicref"]
        except:
            self.write_checkpoints_periodicref = False
        ...
        for n in range(self.num_coupling_surf):
            ...
            for i in range(len(self.surface_p_ids[n])):
                ...
                df_mid_ += self.pbs.timefac * self.pbs.vf.flux(
                    self.pbs.var_u, ds_p, F=self.pbs.ki.F(self.pbs.us_mid, ext=True)
                )

                # add to solid rhs contributions
                self.work_coupling += self.pbs.vf.deltaW_ext_neumann_normal_cur(
                    self.coupfuncs[-1], ds_p, F=self.pbs.ki.F(self.pbs.u, ext=True)
                )
                self.work_coupling_old += self.pbs.vf.deltaW_ext_neumann_normal_cur(
                    self.coupfuncs_old[-1], ds_p, F=self.pbs.ki.F(self.pbs.u_old, ext=True)
                )
                self.work_coupling_mid += self.pbs.vf.deltaW_ext_neumann_normal_cur(
                    self.coupfuncs_mid[-1], ds_p, F=self.pbs.ki.F(self.pbs.us_mid, ext=True)
                )
                ...
                if self.pbs.mat_growth[n]:
                    ...
                    if growth_settrig == "fibstretch":
                        growth_thresolds.append(
                            self.pbs.ma[n].fibstretch_e(self.pbs.ki.C(self.pbs.u), self.pbs.theta, self.pbs.fib_func[0])
                        )
                    elif growth_settrig == "volstress":
                        growth_thresolds.append(
                            tr(
                                self.pbs.ma[n].M_e(
                                    self.pbs.u, self.pbs.p, self.pbs.ki.C(self.pbs.u), ivar=self.pbs.internalvars
                                )
                            )
                        )
                    else:
    ...
    def set_problem_vector_matrix_structures_coupling(self):
        ...
        for n in range(self.num_coupling_surf):
            ...
            nds_vq = np.array(
                self.pbs.V_u.dofmap.index_map.local_to_global(np.asarray(nds_vq_local, dtype=np.int32)), dtype=np.int32
            )
            ...
            nds_p_local = fem.locate_dofs_topological(
                self.pbs.V_u,
                self.pbs.io.mesh.topology.dim - 1,
                self.pbs.io.mt_b1.indices[np.isin(self.pbs.io.mt_b1.values, self.surface_p_ids[n])],
            )
            nds_p = np.array(
                self.pbs.V_u.dofmap.index_map.local_to_global(np.asarray(nds_p_local, dtype=np.int32)), dtype=np.int32
            )
    ...
    def assemble_residual(self, t, subsolver=None):
        ...
        if self.coupling_type == "monolithic_lagrange":
            ...
            self.pb0.cardvasc0D.set_pressure_fem(
                self.LM, list(range(self.num_coupling_surf)), self.pr0D, self.coupfuncs
            )
    ...
    def assemble_stiffness(self, t, subsolver=None):
        ...
        for i in range(len(self.col_ids)):
            ...
            self.K_us.setValues(
                self.dofs_coupling_p[i], self.col_ids[i], self.k_us_subvec[i].array, addv=PETSc.InsertMode.INSERT
            )
        ...
        for i in range(len(self.row_ids)):
            ...
            self.K_su.setValues(
                self.row_ids[i], self.dofs_coupling_vq[i], self.k_su_subvec[i].array, addv=PETSc.InsertMode.INSERT
            )
    ...
    def get_index_sets(self, isoptions={}):
        ...
        if self.pbs.incompressible_2field:
            ...
            iset_p = PETSc.IS().createStride(
                self.pbs.p.x.petsc_vec.getLocalSize(), first=offset_p, step=1, comm=self.comm
            )
    ...
    def evaluate_initial(self):
        ...
        if self.coupling_type == "monolithic_direct":
            self.pb0.cardvasc0D.set_pressure_fem(
                self.pb0.s_old, self.pb0.cardvasc0D.v_ids, self.pr0D, self.coupfuncs_old
            )
