...
class FluidmechanicsConstraintProblem(problem_base):
    def __init__(
        ...
        for i in range(self.num_coupling_surf):
            ...
            if "scales" in self.coupling_params["constraint_physics"][i]:
                ...
                self.vq_scales.append([1.0] * len(self.coupling_params["constraint_physics"][i]["id"]))
        ...
        for n in range(self.num_coupling_surf):
            ...
            for i in range(len(self.surface_vq_ids[n])):
                if self.btype_vq[n][i] == "ext":
                    ...
                    raise NameError(
                        "Unknown boundary type for constraint! Can only be 'ext' (external) or 'int' (internal)."
                    )
                ...
                cq_ += self.vq_scales[n][i] * self.pbf.vf.flux(
                    self.pbf.v, ds_vq, w=self.pbf.alevar["w"], F=self.pbf.alevar["Fale"], fcts=fct_side
                )
                cq_old_ += self.vq_scales[n][i] * self.pbf.vf.flux(
                    self.pbf.v_old, ds_vq, w=self.pbf.alevar["w_old"], F=self.pbf.alevar["Fale_old"], fcts=fct_side
                )
                ...
                if self.coupling_params["multiplier_physics"][n]["type"] == "pressure":
                    ...
                    if self.coupling_params["multiplier_physics"][n]["dir"] == "iso":
                        ...
                        params_ = {
                            "h0": h0,
                            "model": memmodel,
                            "active_stress": {"dir": "cl", "omega": omega, "iota": iota, "gamma": gamma},
                        }
                    else:
                    ...
                    self.power_coupling += self.pbf.vf.deltaW_ext_membrane(
                        self.pbf.ki.F(self.pbf.ufluid),
                        self.pbf.ki.Fdot(self.pbf.v),
                        None,
                        params_,
                        ds_p,
                        ivar=ivar_,
                        fibfnc=self.pbf.fib_func,
                        wallfield=self.pbf.wallfields[n],
                        actweight=self.pbf.actweights[-1],
                        returnquantity="active_stress_power",
                    )
                    self.power_coupling_old += self.pbf.vf.deltaW_ext_membrane(
                        self.pbf.ki.F(self.pbf.uf_old),
                        self.pbf.ki.Fdot(self.pbf.v_old),
                        None,
                        params_,
                        ds_p,
                        ivar=ivar_old_,
                    ...
                    self.power_coupling_mid += self.pbf.vf.deltaW_ext_membrane(
                        self.pbf.ki.F(self.pbf.ufluid_mid),
                        self.pbf.ki.Fdot(self.pbf.vel_mid),
                        None,
                        params_,
                        ds_p,
                        ivar=ivar_mid_,
                    ...
                    df_mid_ += self.pbf.timefac * self.pbf.vf.deltaW_ext_membrane(
                        self.pbf.ki.F(self.pbf.ufluid_mid),
                        self.pbf.ki.Fdot(self.pbf.vel_mid),
                        None,
                        params_,
                    ...
                    df_ += -self.pbf.timefac * self.pbf.vf.deltaW_ext_robin_valve_deriv_visc(
                        self.pbf.v, ds_p, fcts=fct_side, w=self.pbf.alevar["w"], F=self.pbf.alevar["Fale"]
                    )
                    df_mid_ += -self.pbf.timefac * self.pbf.vf.deltaW_ext_robin_valve_deriv_visc(
                        self.pbf.vel_mid, ds_p, fcts=fct_side, w=self.pbf.alevar["w_mid"], F=self.pbf.alevar["Fale_mid"]
                    )
    ...
    def set_problem_residual_jacobian_forms_coupling(self):
        ...
        for i in range(self.num_coupling_surf):
            if self.on_subdomain[i]:
                ...
                em_u = {
                    self.io.mesh: self.pbf.io.submshes_emap[self.coupling_params["constraint_physics"][i]["domain"]][1]
                }
            else:
    ...
    def assemble_residual(self, t, subsolver=None):
        ...
        for i in range(ls, le):
            if self.have_regularization:
                self.r_lm[i] = (
                    self.alpha_reg[i] * (self.constr[i] - self.constr_val[i])
                    + (1.0 - self.alpha_reg[i]) * self.kp_reg[i] * self.LM[i]
                )
            else:
    ...
    def read_restart(self, sname, N):
        ...
        # LM data
        if N > 0:
            restart_data = np.loadtxt(
                self.pbf.io.output_path + "/checkpoint_" + sname + "_lm_" + str(N) + ".txt", ndmin=1
            )
