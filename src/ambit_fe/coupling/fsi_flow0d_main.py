...
class FSIFlow0DProblem(FSIProblem, problem_base):
    def __init__(
        ...
        try:
            self.fsi_governing_type = self.coupling_params["fsi_governing_type"]
        except:
            self.fsi_governing_type = "solid_governed"

        try:
            self.zero_lm_boundary = self.coupling_params["zero_lm_boundary"]
        except:
            self.zero_lm_boundary = False
        ...
        self.V_lm = fem.functionspace(
            self.io.msh_emap_lm[0], ("Lagrange", self.pbs.order_disp, (self.io.msh_emap_lm[0].geometry.dim,))
        )
        ...
        if self.zero_lm_boundary:  # TODO: Seems to not work properly!
            self.io.msh_emap_lm[0].topology.create_connectivity(
                self.io.msh_emap_lm[0].topology.dim - 1, self.io.msh_emap_lm[0].topology.dim
            )
    ...
    def solve_initial_state(self):
        ...
        if self.pb.pbs.timint != "static" and self.pb.pbase.restart_step == 0:
            ...
            utilities.print_status(
                "Setting forms and solving for consistent initial solid acceleration...", self.pb.comm, e=" "
            )
        ...
        if (
            ...
            ts = time.time()
            utilities.print_status(
                "Setting forms and solving for consistent initial fluid acceleration...", self.pb.comm, e=" "
            )
