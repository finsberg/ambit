...
class constitutive:
    ...
    def sigma_vonmises(self, u_, p_, v_, ivar):
        return ufl.sqrt(3.0 * 0.5 * ufl.inner(self.sigma_dev(u_, p_, v_, ivar), self.sigma_dev(u_, p_, v_, ivar)))
    ...
    def res_dtheta_growth(self, u_, p_, v_, ivar, theta_old_, dt, thres, rquant):
        ...
        threshold = ufl.conditional(
            ufl.gt(thres, self.gandrparams["growth_thres"]), (1.0 + omega) * thres, self.gandrparams["growth_thres"]
        )
    ...
    def dS_dFg(self, u_, p_, v_, ivar, theta_old_, dt):
        ...
        Cmat_e = ufl.dot(
            self.F_g(theta_), ufl.dot(self.F_g(theta_), ufl.dot(Cmat, ufl.dot(self.F_g(theta_).T, self.F_g(theta_).T)))
        )
        ...
        Fginv_outertop_Fginv = ufl.as_tensor(
            ufl.inv(self.F_g(theta_))[i, k] * ufl.inv(self.F_g(theta_))[j, l], (i, j, k, l)
        )
        FginvT_outertop_Ce = ufl.as_tensor(
            ufl.inv(self.F_g(theta_)).T[i, k] * self.C_e(self.kin.C(u_), theta_)[j, l], (i, j, k, l)
        )
        Ce_outerbot_FginvT = ufl.as_tensor(
            self.C_e(self.kin.C(u_), theta_)[i, l] * ufl.inv(self.F_g(theta_)).T[j, k], (i, j, k, l)
        )

        Cmat_e_with_C_e = 0.5 * ufl.as_tensor(
            Cmat_e[i, j, m, n] * (FginvT_outertop_Ce[m, n, k, l] + Ce_outerbot_FginvT[m, n, k, l]), (i, j, k, l)
        )

        Fginv_outertop_Fginv_with_Cmat_e_with_C_e = ufl.as_tensor(
            Fginv_outertop_Fginv[i, j, m, n] * Cmat_e_with_C_e[m, n, k, l], (i, j, k, l)
        )
    ...
    def Cremod(self, u_, p_, v_, ivar, theta_old_, dt, thres):
        ...
        Cremod = (
            2.0
            * dphi_dtheta_
            * ufl.as_tensor(dtheta_dC_[i, j] * (self.stress_remod - self.stress_base)[k, l], (i, j, k, l))
        )
