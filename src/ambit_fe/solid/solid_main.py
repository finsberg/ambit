...
class SolidmechanicsProblem(problem_base):
    def __init__(
        self, pbase, io_params, time_params, fem_params, constitutive_models, bc_dict, time_curves, io, mor_params={}
    ):
        self.pbase = pbase
        ...
        try:
            self.domain_ids = self.io.io_params["domain_ids_solid"]
        except:
        ...
        self.V_tensor = fem.functionspace(
            self.io.mesh, ("Lagrange", self.order_disp, (self.io.mesh.geometry.dim, self.io.mesh.geometry.dim))
        )
        ...
        self.Vd_tensor = fem.functionspace(
            self.io.mesh, (self.dg_type, self.order_disp - 1, (self.io.mesh.geometry.dim, self.io.mesh.geometry.dim))
        )
        ...
        try:
            self.volume_laplace = io_params["volume_laplace"]
        except:
            self.volume_laplace = []
        ...
        for n in range(self.num_domains):
            ...
            if "growth" in self.constitutive_models["MAT" + str(n + 1)].keys():
                ...
                self.internalvars["theta"], self.internalvars_old["theta"], self.internalvars_mid["theta"] = (
                    self.theta,
                    self.theta_old,
            ...
            if "plastic" in self.constitutive_models["MAT" + str(n + 1)].keys():
                ...
                self.localsolve = True
                self.internalvars["e_plast"], self.internalvars_old["e_plast"], self.internalvars_mid["e_plast"] = (
                    self.F_plast,
                    self.F_plast_old,
        ...
        try:
            self.lin_remod_full = fem_params["lin_remodeling_full"]
        except:
            self.lin_remod_full = True
        ...
        self.vf = solid_variationalform.variationalform(
            self.var_u, var_p=self.var_p, du=self.du, dp=self.dp, n0=self.io.n0, x_ref=self.x_ref
        )
    ...
    def set_variational_forms(self):
        ...
        for n, M in enumerate(self.domain_ids):
            ...
            self.deltaW_int += self.vf.deltaW_int(
                self.ma[n].S(self.u, self.p, self.vel, ivar=self.internalvars), self.ki.F(self.u), self.dx(M)
            )
            self.deltaW_int_old += self.vf.deltaW_int(
                self.ma[n].S(self.u_old, self.p_old, self.v_old, ivar=self.internalvars_old),
                self.ki.F(self.u_old),
                self.dx(M),
            )
            self.deltaW_int_mid += self.vf.deltaW_int(
                self.ma[n].S(self.us_mid, self.ps_mid, self.vel_mid, ivar=self.internalvars_mid),
            ...
            if self.mat_growth[n]:
                J, J_old, J_mid = (
                    ...
                    self.ma[n].J_e(self.us_mid, self.timefac * self.theta + (1.0 - self.timefac) * self.theta_old),
                )
            else:
                J, J_old, J_mid = self.ki.J(self.u), self.ki.J(self.u_old), self.ki.J(self.us_mid)
        ...
        if "robin" in self.bc_dict.keys():
            ...
            w_robin_old = self.bc.robin_bcs(
                self.bc_dict["robin"], self.u_old, self.v_old, self.bmeasures, u_pre=self.u_pre
            )
            w_robin_mid = self.bc.robin_bcs(
                self.bc_dict["robin"], self.us_mid, self.vel_mid, self.bmeasures, u_pre=self.u_pre
            )
        if "membrane" in self.bc_dict.keys():
            w_membrane, self.idmem, self.bstress, self.bstrainenergy, self.bintpower = self.bc.membranesurf_bcs(
                self.bc_dict["membrane"], self.u, self.vel, self.acc, self.bmeasures
            )
            w_membrane_old, _, _, _, _ = self.bc.membranesurf_bcs(
                self.bc_dict["membrane"], self.u_old, self.v_old, self.a_old, self.bmeasures
            )
            w_membrane_mid, _, _, _, _ = self.bc.membranesurf_bcs(
                self.bc_dict["membrane"], self.us_mid, self.vel_mid, self.acc_mid, self.bmeasures
            )
        ...
        if self.prestress_initial or self.prestress_initial_only:
            ...
            for n, M in enumerate(self.domain_ids):
                self.deltaW_prestr_int += self.vf.deltaW_int(
                    self.ma_prestr[n].S(self.u, self.p, self.vel, ivar=self.internalvars), self.ki.F(self.u), self.dx(M)
                )
            # boundary conditions
            ...
            if "robin" in bc_dict_prestr.keys():
                w_robin_prestr = bc_prestr.robin_bcs(
                    bc_dict_prestr["robin"], self.u, self.vel, self.bmeasures, u_pre=self.u_pre
                )
        ...
        for n, M in enumerate(self.domain_ids):
            ...
            if self.ti.eval_nonlin_terms == "midpoint":
                Cmat, Cmat_v = self.ma[n].S(
                    self.us_mid, self.ps_mid, self.vel_mid, ivar=self.internalvars_mid, returnquantity="tangent"
                )
            ...
            if (
                ...
                Cgrowth = self.ma[n].Cgrowth(
                    self.u, self.p, self.vel, self.internalvars, self.theta_old, self.pbase.dt, self.growth_thres
                )
        ...
        if self.incompressible_2field:
            self.weakform_lin_up, self.weakform_lin_pu, self.weakform_lin_pp = (
                ufl.as_ufl(0),
                ufl.as_ufl(0),
                ufl.as_ufl(0),
            )
            ...
            for n, M in enumerate(self.domain_ids):
                ...
                if self.ti.eval_nonlin_terms == "midpoint":
                    Cmat_p = ufl.diff(
                        self.ma[n].S(self.us_mid, self.ps_mid, self.vel_mid, ivar=self.internalvars_mid), self.p
                    )
                ...
                if (
                    ...
                    # elastic and viscous material tangent operator
                    Cmat, Cmat_v = self.ma[n].S(
                        self.u, self.p, self.vel, ivar=self.internalvars, returnquantity="tangent"
                    )
                        ...
                        if self.activemodel[n] == "active_fiber":
                            if self.mat_growth[n]:
                                lam_fib_old = self.ma[n].fibstretch_e(
                                    self.ki.C(self.u_old), self.theta_old, self.fib_func[0]
                                )
                            else:
                ...
                if self.mat_active_stress[n]:
                    if self.mat_active_stress_type[n] == "ode":
                        ...
                        if self.actstress[na].frankstarling:
                            if self.activemodel[n] == "active_fiber":
                                if self.mat_growth[n]:
                                    lam_fib = self.ma[n].fibstretch_e(self.ki.C(self.u), self.theta, self.fib_func[0])
                                else:
                                    lam_fib = self.ki.fibstretch(self.u, self.fib_func[0])
                            elif self.activemodel[n] == "active_crossfiber":
                                if self.mat_growth[n]:
                                    lam_fib = self.ma[n].crossfibstretch_e(
                                        self.ki.C(self.u), self.theta, self.fib_func[0]
                                    )
                                else:
                        ...
                        self.tau_a_.append(
                            self.actstress[na].tau_act(self.tau_a_old, self.pbase.dt, lam=lam_fib, amp_old=self.amp_old)
                        )
                        na += 1
                    if self.mat_active_stress_type[n] == "prescribed":
    ...
    def compute_strain_energy_power(self, N, t):
        ...
        for n, M in enumerate(self.domain_ids):
            se_all += self.ma[n].S(
                self.u, self.p, self.vel, ivar=self.internalvars, returnquantity="strainenergy"
            ) * self.dx(M)
            ip_all += ufl.inner(
                self.ma[n].S(self.u, self.p, self.vel, ivar=self.internalvars), self.ki.Edot(self.u, self.vel)
            ) * self.dx(M)
        ...
        dbcs_laplace.append(
            fem.dirichletbc(
                ...
                fem.locate_dofs_topological(
                    self.V_u, 2, self.io.mt_b1.indices[np.isin(self.io.mt_b1.values, self.volume_laplace)]
                ),
            )
        )
    ...
    def get_index_sets(self, isoptions={}):
        ...
        iset_p = PETSc.IS().createStride(
            self.p.x.petsc_vec.getLocalSize(), first=offset_p, step=1, comm=self.pbase.comm
        )
    ...
    def solve_initial_state(self):
        ...
        if self.pb.timint != "static" and self.pb.pbase.restart_step == 0:
            ...
            utilities.print_status(
                "Setting forms and solving for consistent initial acceleration...", self.pb.pbase.comm, e=" "
            )
    ...
    def solve_initial_prestress(self):
        ...
        if self.pb.prestress_initial_only:
            ...
            self.pb.io.writefunction(
                self.pb.u_pre, self.pb.io.output_path_pre + "/results_" + self.pb.pbase.simname + "_displacement_pre"
            )
            if self.pb.incompressible_2field:
                self.pb.io.writefunction(
                    self.pb.p, self.pb.io.output_path_pre + "/results_" + self.pb.pbase.simname + "_pressure_pre"
                )
            utilities.print_status(
