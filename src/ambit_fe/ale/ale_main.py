...
class AleProblem(problem_base):
    def __init__(
        ...
        self.Vd_tensor = fem.functionspace(
            self.io.mesh, (dg_type, self.order_disp - 1, (self.io.mesh.geometry.dim, self.io.mesh.geometry.dim))
        )
        ...
        self.bc = boundaryconditions.boundary_cond(
            self.io, fem_params=fem_params, vf=self.vf, ti=self.ti, V_field=self.V_d, Vdisc_scalar=self.Vd_scalar
        )
