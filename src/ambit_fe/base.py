...
class problem_base:
    def __init__(self, io_params, time_params, comm=None, comm_sq=None):
        ...
        try:
            self.restart_step = io_params["restart_step"]
        except:
            self.restart_step = 0

        try:
            self.numstep_stop = time_params["numstep_stop"]
        except:
            self.numstep_stop = self.numstep
        ...
        try:
            self.residual_scale = time_params["residual_scale"]
        except:
            self.residual_scale = []
    ...
    def scale_jacobian_list(self, Klist):
        for n in range(len(Klist)):
            for m in range(len(Klist)):
                if Klist[n][m] is not None:
                    Klist[n][m].scale(self.pbase.residual_scale[n])
...
class solver_base:
    ...
    def evaluate_assemble_system_initial(self, subsolver=None):
        ...
        )  # note: subsolver only passed to stiffness eval to get correct sparsity pattern of lm-lm block
    ...
    def print_final(self):
        utilities.print_status(
            ...
            % (time.time() - self.starttime, (time.time() - self.starttime) / 60.0),
            self.pb.comm,
        )

        utilities.print_status(
            "{:<55s}{:<1.4f}{:<2s}".format("Total solution time of all steps: ", self.wt, " s"), self.pb.comm
        )
        utilities.print_status(
            "{:<55s}{:<1.4f}{:<2s}".format("Average solution time per time step: ", self.wt / self.Nfinal, " s"),
            self.pb.comm,
        )
        utilities.print_status(
            "{:<55s}{:<1.4f}{:<2s}".format("Maximum solution time of a time step: ", max(self.wt_), " s"), self.pb.comm
        )
        ...
        utilities.print_status(
            "{:<55s}{:<1.1f}".format("Average number of nonlinear iterations per time step: ", self.ni / self.Nfinal),
            self.pb.comm,
        )
        utilities.print_status(
            "{:<55s}{:<1d}".format("Maximum number of nonlinear iterations in a time step: ", max(self.ni_)),
            self.pb.comm,
        )
        ...
        if self.solnln.solvetype[0] == "iterative":
            ...
            utilities.print_status(
                "{:<55s}{:<1.1f}".format("Average number of linear iterations per time step: ", self.li / self.Nfinal),
                self.pb.comm,
            )
            utilities.print_status(
                "{:<55s}{:<1d}".format("Maximum number of linear iterations in a time step: ", max(self.li_)),
                self.pb.comm,
            )
            utilities.print_status(
                "{:<55s}{:<1.1f}".format("Average number of linear iterations per solve: ", self.li / self.ni),
                self.pb.comm,
            )
            utilities.print_status(
                "{:<55s}{:<1d}".format("Maximum number of linear iterations in a solve: ", max(self.solnln.li_s)),
                self.pb.comm,
            )
    ...
    def update_counters(self, wt, t):
        ...
        if "counters" in self.pb.pbase.results_to_write:
            ...
            if self.pb.comm.rank == 0:
                f = open(
                    self.pb.pbase.output_path + "/results_" + self.pb.pbase.simname + "_wt_ni_li_counters.txt", mode
                )
