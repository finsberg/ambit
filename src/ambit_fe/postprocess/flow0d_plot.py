...
parser.add_argument(
    "-cf", "--calcfunc", dest="cf", action="store", type=lambda x: bool(distutils.util.strtobool(x)), default=True
...
parser.add_argument(
    "-mgr", "--multgandr", dest="mgr", action="store", type=lambda x: bool(distutils.util.strtobool(x)), default=False
...
parser.add_argument(
    "-V0", "--Vinitial", dest="V0", nargs=5, action="store", type=float, default=[113.25e3, 150e3, 50e3, 50e3, 0e3]
)  # initial chamber vols: order is lv,rv,la,ra,ao
parser.add_argument(
    "-png", "--pngexport", dest="png", action="store", type=lambda x: bool(distutils.util.strtobool(x)), default=True
)  # whether png files should be created for the plots
parser.add_argument(
    "-plt", "--genplots", dest="plt", action="store", type=lambda x: bool(distutils.util.strtobool(x)), default=True
)  # whether plots should be generated
parser.add_argument(
    "-ext", "--extplot", dest="ext", action="store", type=lambda x: bool(distutils.util.strtobool(x)), default=False
...
def postprocess0D(
    ...
    if iscirculation:
        ...
        if calculate_function_params:
            ...
            for ch in ["v_l", "v_r"]:
                ...
                vol = np.loadtxt(
                    path + "/results_" + sname + "_V_" + ch + ".txt", skiprows=max(0, numdata - nstep_cycl)
                )
                ...
                pres = np.loadtxt(
                    path + "/results_" + sname + "_p_" + ch + ".txt", skiprows=max(0, numdata - nstep_cycl)
                )
                ...
                if ch == "v_l":
                    fluxout = np.loadtxt(
                        path + "/results_" + sname + "_q_vout_l.txt", skiprows=max(0, numdata - nstep_cycl)
                    )
                    fluxin = np.loadtxt(
                        path + "/results_" + sname + "_q_vin_l.txt", skiprows=max(0, numdata - nstep_cycl)
                    )
                if ch == "v_r":
                    fluxout = np.loadtxt(
                        path + "/results_" + sname + "_q_vout_r.txt", skiprows=max(0, numdata - nstep_cycl)
                    )
                    fluxin = np.loadtxt(
                        path + "/results_" + sname + "_q_vin_r.txt", skiprows=max(0, numdata - nstep_cycl)
                    )
            ...
            for pc in ["ar_sys", "ar_pul"]:
                par = np.loadtxt(
                    path + "/results_" + sname + "_p_" + pc + ".txt", skiprows=max(0, numdata - nstep_cycl)
                )
    ...
    if generate_plots:
        ...
        for g in range(len(groups)):
            ...
            subprocess.call(
                ["cp", str(fpath) + "/flow0d_gnuplot_template.p", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
            )
            subprocess.call(
                [
                    "sed",
            ...
            subprocess.call(
                ["sed", "-i", "s#__FILEDIR__#" + path + "#", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
            )
            ...
            if x2value != "":
                subprocess.call(["sed", "-i", "s/#__HAVEX2__//", path + "/plot_" + list(groups[g].keys())[0] + ".p"])
            if y2value != "":
                subprocess.call(["sed", "-i", "s/#__HAVEY2__//", path + "/plot_" + list(groups[g].keys())[0] + ".p"])

            # axis segments - x
            subprocess.call(
                ["sed", "-i", "s/__X1S__/" + str(x_s) + "/", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
            )
            subprocess.call(
                [
                    "sed",
            ...
            # axis segments - y
            subprocess.call(
                ["sed", "-i", "s/__Y1S__/" + str(y_s) + "/", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
            )
            subprocess.call(
                [
                    "sed",
            ...
            # units
            subprocess.call(
                ["sed", "-i", "s#__X1UNIT__#" + x1unit + "#", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
            )
            subprocess.call(
                ["sed", "-i", "s#__Y1UNIT__#" + y1unit + "#", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
            )
            if x2unit != "":
                subprocess.call(
                    ["sed", "-i", "s#__X2UNIT__#" + x2unit + "#", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
                )
            if y2unit != "":
                subprocess.call(
                    ["sed", "-i", "s#__Y2UNIT__#" + y2unit + "#", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
                )
            # values
            subprocess.call(
                ["sed", "-i", "s#__X1VALUE__#" + x1value + "#", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
            )
            subprocess.call(
                ["sed", "-i", "s#__Y1VALUE__#" + y1value + "#", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
            )
            if x2value != "":
                subprocess.call(
                    ["sed", "-i", "s#__X2VALUE__#" + x2value + "#", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
                )
            if y2value != "":
                subprocess.call(
                    ["sed", "-i", "s#__Y2VALUE__#" + y2value + "#", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
                )
            # scales
            subprocess.call(
                ["sed", "-i", "s/__XSCALE__/" + str(xscale) + "/g", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
            )
            subprocess.call(
                ["sed", "-i", "s/__YSCALE__/" + str(yscale) + "/g", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
            )
            ...
            subprocess.call(
                ["sed", "-i", "s/__SAMPLEN__/" + str(sl) + "/g", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
            )
            subprocess.call(
                ["sed", "-i", "s/__SAMPWID__/" + str(swd) + "/g", path + "/plot_" + list(groups[g].keys())[0] + ".p"]
            )
...
def postprocess_groups_syspul(groups, coronarymodel=None, indpertaftercyl=0, multiscalegandr=False):
    ...
    groups.append(
        {
            "pres_time_sys_l": ["p_at_l", "p_v_l", "p_ar_sys", "p_ard_sys", "p_ven_sys"],
            "tex": [
                "$p_{\\\mathrm{at}}^{\\\ell}$",
                "$p_{\\\mathrm{v}}^{\\\ell}$",
    ...
    groups.append(
        {
            "flux_time_sys_l": ["q_vin_l", "q_vout_l", "q_ar_sys", "q_ven1_sys", "q_ven2_sys"],
            "tex": [
                "$q_{\\\mathrm{v,in}}^{\\\ell}$",
                "$q_{\\\mathrm{v,out}}^{\\\ell}$",
    ...
    if indpertaftercyl > 0:  # for comparison of healthy/baseline and perturbed states
        if multiscalegandr:
            ...
            groups.append(
                {
                    "pres_vol_v_l_r_PERIODIC": ["pV_v_l_last", "pV_v_r_last", "pV_v_l_baseline", "pV_v_r_baseline"],
                    "tex": [
                        "$p_{\\\mathrm{v}}^{\\\ell}$",
                        "$p_{\\\mathrm{v}}^{r}$",
                        "$p_{\\\mathrm{v}}^{\\\ell,\\\mathrm{ref}}$",
...
def postprocess_groups_syspulcap(groups, indpertaftercyl=0, multiscalegandr=False):
    ...
    groups.append(
        {
            "pres_time_pul_r": ["p_at_r", "p_v_r", "p_ar_pul", "p_cap_pul", "p_ven_pul"],
            "tex": [
                "$p_{\\\mathrm{at}}^{r}$",
                "$p_{\\\mathrm{v}}^{r}$",
                "$p_{\\\mathrm{ar}}^{\\\mathrm{pul}}$",
    ...
    groups.append(
        {
            "flux_time_pul_r": ["q_vin_r", "q_vout_r", "q_ar_pul", "q_cap_pul", "q_ven_pul"],
            "tex": [
                "$q_{\\\mathrm{v,in}}^{r}$",
                "$q_{\\\mathrm{v,out}}^{r}$",
                "$q_{\\\mathrm{ar}}^{\\\mathrm{pul}}$",
...
def postprocess_groups_syspulcaprespir(groups, coronarymodel=None, indpertaftercyl=0, multiscalegandr=False):
    ...
    groups.append(
        {
            "ppO2_time_pul_r": ["ppO2_at_r", "ppO2_v_r", "ppO2_ar_pul", "ppO2_ven_pul", "ppO2_cap_pul"],
            "tex": [
    ...
    groups.append(
        {
            "ppCO2_time_pul_r": ["ppCO2_at_r", "ppCO2_v_r", "ppCO2_ar_pul", "ppCO2_ven_pul", "ppCO2_cap_pul"],
            "tex": [
