...
class boundary_cond:
    def __init__(
        self, io, fem_params=None, vf=None, ti=None, ki=None, ff=None, dim=None, V_field=None, Vdisc_scalar=None
    ):
        self.io = io
        self.vf = vf
        self.ti = ti
    ...
    def dirichlet_bcs(self, bcdict):
        for d in bcdict:
            ...
            if "curve" in d.keys():
                ...
                if d["dir"] == "all":
                    curve_x, curve_y, curve_z = d["curve"][0], d["curve"][1], d["curve"][2]
                else:
                    curve_x, curve_y, curve_z = d["curve"], d["curve"], d["curve"]
                ...
                self.ti.funcs_to_update_vec.append(
                    {func: [self.ti.timecurves(curve_x), self.ti.timecurves(curve_y), self.ti.timecurves(curve_z)]}
                )
            ...
            if d["dir"] == "all":
                self.dbcs.append(
                    fem.dirichletbc(
                        func,
                        fem.locate_dofs_topological(self.V_field, codim, mdata.indices[np.isin(mdata.values, d["id"])]),
                    )
                )

            elif d["dir"] == "x":
                dofs_x = fem.locate_dofs_topological(
                    self.V_field.sub(0), codim, mdata.indices[np.isin(mdata.values, d["id"])]
                )
                self.dbcs.append(fem.dirichletbc(func.sub(0), dofs_x))

            elif d["dir"] == "y":
                dofs_y = fem.locate_dofs_topological(
                    self.V_field.sub(1), codim, mdata.indices[np.isin(mdata.values, d["id"])]
                )
                self.dbcs.append(fem.dirichletbc(func.sub(1), dofs_y))

            elif d["dir"] == "z":
                dofs_z = fem.locate_dofs_topological(
                    self.V_field.sub(2), codim, mdata.indices[np.isin(mdata.values, d["id"])]
                )
                self.dbcs.append(fem.dirichletbc(func.sub(2), dofs_z))

            elif d["dir"] == "2dimX":
                dofs_x = fem.locate_dofs_topological(
                    self.V_field.sub(0), codim, mesh.locate_entities_boundary(self.io.mesh, codim, self.twodimX)
                )
                self.dbcs.append(fem.dirichletbc(func.sub(0), dofs_x))

            elif d["dir"] == "2dimY":
                dofs_y = fem.locate_dofs_topological(
                    self.V_field.sub(1), codim, mesh.locate_entities_boundary(self.io.mesh, codim, self.twodimY)
                )
                self.dbcs.append(fem.dirichletbc(func.sub(1), dofs_y))

            elif d["dir"] == "2dimZ":
                dofs_z = fem.locate_dofs_topological(
                    self.V_field.sub(2), codim, mesh.locate_entities_boundary(self.io.mesh, codim, self.twodimZ)
                )
                self.dbcs.append(fem.dirichletbc(func.sub(2), dofs_z))
            ...
            self.dbcs.append(
                fem.dirichletbc(
                    ...
                    fem.locate_dofs_topological(
                        self.V_field, self.dim, self.io.mt_d.indices[np.isin(self.io.mt_d.values, d["id"])]
                    ),
                )
            )
                ...
                for i in range(len(r["id"])):
                    if dw is None:
                        ...
                        dwddp += self.vf.deltaW_ext_robin_valve(
                            v, ufl.as_ufl(1.0), dS_[dind](r["id"][i]), fcts="+", w=wel_, F=F
                        )
                ...
                for i in range(len(r["id"])):
                    if dw is None:
                        w += self.vf.deltaW_ext_robin_valve_normal_ref(
                            v, beta_[-1], dS_[dind](r["id"][i]), fcts="+", w=wel_, F=F
                        )
                    else:
                        # derivative (for implicit valve law)
