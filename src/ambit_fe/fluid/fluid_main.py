...
class FluidmechanicsProblem(problem_base):
    def __init__(
        ...
        try:
            self.domain_ids = self.io.io_params["domain_ids_fluid"]
        except:
        ...
        try:
            self.fluid_formulation = fem_params["fluid_formulation"]
        except:
            self.fluid_formulation = "nonconservative"

        try:
            self.fluid_governing_type = time_params["fluid_governing_type"]
        except:
            self.fluid_governing_type = "navierstokes_transient"

        try:
            self.stabilization = fem_params["stabilization"]
        except:
            self.stabilization = None

        try:
            self.prestress_initial = fem_params["prestress_initial"]
        except:
            self.prestress_initial = False
        try:
            self.prestress_initial_only = fem_params["prestress_initial_only"]
        except:
            self.prestress_initial_only = False
        try:
            self.prestress_maxtime = fem_params["prestress_maxtime"]
        except:
            self.prestress_maxtime = 1.0
        try:
            self.prestress_numstep = fem_params["prestress_numstep"]
        except:
            self.prestress_numstep = 1
        try:
            self.prestress_dt = fem_params["prestress_dt"]
        except:
            self.prestress_dt = self.prestress_maxtime / self.prestress_numstep
        if "prestress_dt" in fem_params.keys():
            self.prestress_numstep = int(self.prestress_maxtime / self.prestress_dt)
        try:
            self.prestress_ptc = fem_params["prestress_ptc"]
        except:
            self.prestress_ptc = False
        try:
            self.prestress_kinetic = fem_params["prestress_kinetic"]
        except:
            self.prestress_kinetic = "none"
        try:
            self.prestress_from_file = fem_params["prestress_from_file"]
        except:
            self.prestress_from_file = False
        try:
            self.initial_fluid_pressure = fem_params["initial_fluid_pressure"]
        except:
            self.initial_fluid_pressure = []
        ...
        self.have_flux_monitor, self.have_dp_monitor, self.have_robin_valve, self.have_robin_valve_implicit = (
            False,
            False,
            False,
            False,
        ...
        if self.stabilization is None and self.order_vel == self.order_pres:
            raise ValueError(
                "Equal order velocity and pressure interpolation is not recommended for non-stabilized Navier-Stokes!"
            )
        ...
        if bool(self.io.duplicate_mesh_domains):
            ...
            for m, mp in enumerate(self.io.duplicate_mesh_domains):
                self.io.submshes_emap[m + 1] = mesh.create_submesh(
                    self.io.mesh, self.io.mesh.topology.dim, self.io.mt_d.indices[np.isin(self.io.mt_d.values, mp)]
                )[0:2]
            ...
            for m, mp in enumerate(self.io.duplicate_mesh_domains):
                ...
                dxp, bmeasuresp = self.io.create_integration_measures(
                    self.io.submshes_emap[m + 1][0], [self.io.sub_mt_d[m + 1], self.io.sub_mt_b1[m + 1], None]
                )
        ...
        self.V_tensor = fem.functionspace(
            self.io.mesh, ("Lagrange", self.order_vel, (self.io.mesh.geometry.dim, self.io.mesh.geometry.dim))
        )
        ...
        self.Vd_tensor = fem.functionspace(
            self.io.mesh, (dg_type, self.order_vel - 1, (self.io.mesh.geometry.dim, self.io.mesh.geometry.dim))
        )
        ...
        self.V_out_tensor = fem.functionspace(
            self.io.mesh, ("Lagrange", self.mesh_degree, (self.io.mesh.geometry.dim, self.io.mesh.geometry.dim))
        )
        self.V_out_vector = fem.functionspace(
            self.io.mesh, ("Lagrange", self.mesh_degree, (self.io.mesh.geometry.dim,))
        )
        ...
        if self.num_dupl > 1:
            ...
            self.p_, self.dp_, self.var_p_, self.p_old_ = (
                list(self.p__.values()),
                list(self.dp__.values()),
                list(self.var_p__.values()),
                list(self.p_old__.values()),
            )
        else:
        ...
        if not bool(self.alevar):
            ...
            self.vf = fluid_variationalform.variationalform(
                self.var_v, var_p=self.var_p_, n0=self.io.n0, formulation=self.fluid_formulation
            )
        ...
        if bool(self.io.fiber_data) and (
            self.pbase.problem_type == "fluid_ale"
            or self.pbase.problem_type == "fluid_ale_flow0d"
            or self.pbase.problem_type == "fluid_ale_constraint"
        ):  # only for FrSI problem
            self.fibarray = ["circ"]
            if len(self.io.fiber_data) > 1:
                self.fibarray.append("long")
    ...
    def set_variational_forms(self):
        ...
        if self.num_dupl > 1:
            for m, mp in enumerate(self.io.duplicate_mesh_domains):
                self.pf_mid__[m + 1] = self.timefac * self.p__[m + 1] + (1.0 - self.timefac) * self.p_old__[m + 1]
            # make list
        ...
        for n, M in enumerate(self.domain_ids):
            ...
            if self.fluid_governing_type == "navierstokes_transient":
                self.deltaW_kin += self.vf.deltaW_kin_navierstokes_transient(
                    self.acc, self.v, self.rho[n], self.dx(M), w=self.alevar["w"], F=self.alevar["Fale"]
                )
                self.deltaW_kin_old += self.vf.deltaW_kin_navierstokes_transient(
                    self.a_old, self.v_old, self.rho[n], self.dx(M), w=self.alevar["w_old"], F=self.alevar["Fale_old"]
                )
                ...
                self.deltaW_kin += self.vf.deltaW_kin_navierstokes_steady(
                    self.v, self.rho[n], self.dx(M), w=self.alevar["w"], F=self.alevar["Fale"]
                )
                self.deltaW_kin_old += self.vf.deltaW_kin_navierstokes_steady(
                    self.v_old, self.rho[n], self.dx(M), w=self.alevar["w_old"], F=self.alevar["Fale_old"]
                )
                self.deltaW_kin_mid += self.vf.deltaW_kin_navierstokes_steady(
                    self.vel_mid, self.rho[n], self.dx(M), w=self.alevar["w_mid"], F=self.alevar["Fale_mid"]
                )
            ...
            self.deltaW_int += self.vf.deltaW_int(
                self.ma[n].sigma(self.v, self.p_[j], F=self.alevar["Fale"]), self.dx(M), F=self.alevar["Fale"]
            )
            self.deltaW_int_old += self.vf.deltaW_int(
                self.ma[n].sigma(self.v_old, self.p_old_[j], F=self.alevar["Fale_old"]),
                self.dx(M),
                F=self.alevar["Fale_old"],
            )
            self.deltaW_int_mid += self.vf.deltaW_int(
                self.ma[n].sigma(self.vel_mid, self.pf_mid_[j], F=self.alevar["Fale_mid"]),
                self.dx(M),
                F=self.alevar["Fale_mid"],
            )
            ...
            self.deltaW_p.append(
                self.vf.deltaW_int_pres(self.v, self.var_p_[j], self.dx_p[j](M), F=self.alevar["Fale"])
            )
            self.deltaW_p_old.append(
                self.vf.deltaW_int_pres(self.v_old, self.var_p_[j], self.dx_p[j](M), F=self.alevar["Fale_old"])
            )
            self.deltaW_p_mid.append(
                self.vf.deltaW_int_pres(self.vel_mid, self.var_p_[j], self.dx_p[j](M), F=self.alevar["Fale_mid"])
            )
            ...
            self.Re_c_old += self.vf.re_c(
                self.rho[n], self.v_old, self.dx(M), w=self.alevar["w_old"], F=self.alevar["Fale_old"]
            )
            self.Re_c_mid += self.vf.re_c(
                self.rho[n], self.vel_mid, self.dx(M), w=self.alevar["w_mid"], F=self.alevar["Fale_mid"]
            )

            self.Re_ktilde += self.vf.re_ktilde(
                self.rho[n], self.v, self.dx(M), w=self.alevar["w"], F=self.alevar["Fale"]
            )
        ...
        w_neumann, w_body, w_robin, w_stabneumann, w_stabneumann_mod, w_robin_valve, w_membrane = (
            ufl.as_ufl(0),
            ufl.as_ufl(0),
            ufl.as_ufl(0),
            ufl.as_ufl(0),
        ...
        (
            ...
            w_robin_old,
            w_stabneumann_old,
            w_stabneumann_mod_old,
            w_robin_valve_old,
            w_membrane_old,
        ) = ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0)
        (
            w_neumann_mid,
            w_body_mid,
            w_robin_mid,
            w_stabneumann_mid,
            w_stabneumann_mod_mid,
            w_robin_valve_mid,
            w_membrane_mid,
        ) = ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0)
        ...
        if "robin" in self.bc_dict.keys():
            w_robin = self.bc.robin_bcs(
                self.bc_dict["robin"], self.ufluid, self.v, self.bmeasures, F=self.alevar["Fale"], u_pre=self.uf_pre
            )
        ...
        if "stabilized_neumann" in self.bc_dict.keys():
            w_stabneumann = self.bc.stabilized_neumann_bcs(
                self.bc_dict["stabilized_neumann"], self.v, self.bmeasures, wel=self.alevar["w"], F=self.alevar["Fale"]
            )
            w_stabneumann_old = self.bc.stabilized_neumann_bcs(
                self.bc_dict["stabilized_neumann"],
                self.v_old,
                self.bmeasures,
                wel=self.alevar["w_old"],
                F=self.alevar["Fale_old"],
            )
            w_stabneumann_mid = self.bc.stabilized_neumann_bcs(
                self.bc_dict["stabilized_neumann"],
                self.vel_mid,
                self.bmeasures,
        ...
        if "stabilized_neumann_mod" in self.bc_dict.keys():
            w_stabneumann_mod = self.bc.stabilized_neumann_mod_bcs(
                self.bc_dict["stabilized_neumann_mod"],
                self.v,
                self.bmeasures,
                wel=self.alevar["w"],
                F=self.alevar["Fale"],
            )
            w_stabneumann_mod_old = self.bc.stabilized_neumann_mod_bcs(
                self.bc_dict["stabilized_neumann_mod"],
                self.v_old,
                self.bmeasures,
                wel=self.alevar["w_old"],
            ...
            w_stabneumann_mod_mid = self.bc.stabilized_neumann_mod_bcs(
                self.bc_dict["stabilized_neumann_mod"],
                self.vel_mid,
                self.bmeasures,
                wel=self.alevar["w_mid"],
        ...
        if "robin_valve" in self.bc_dict.keys():
            ...
            w_robin_valve_old = self.bc.robin_valve_bcs(
                self.bc_dict["robin_valve"],
                self.v_old,
                self.beta_valve_old,
                [self.bmeasures[2]],
            ...
            for m in range(len(self.bc_dict["robin_valve"])):
                beta_min, beta_max = (
                    self.bc_dict["robin_valve"][m]["beta_min"],
                    self.bc_dict["robin_valve"][m]["beta_max"],
                )
        ...
        if "robin_valve_implicit" in self.bc_dict.keys():
            ...
            self.z, self.z_old = (
                PETSc.Vec().createMPI(size=self.num_valve_coupling_surf),
                PETSc.Vec().createMPI(size=self.num_valve_coupling_surf),
            )
        ...
        if "flux_monitor" in self.bc_dict.keys():
            ...
            self.bc.flux_monitor_bcs(
                self.bc_dict["flux_monitor"], self.v, self.q_, wel=self.alevar["w"], F=self.alevar["Fale"]
            )
            self.bc.flux_monitor_bcs(
                self.bc_dict["flux_monitor"],
                self.v_old,
                self.q_old_,
                wel=self.alevar["w_old"],
                F=self.alevar["Fale_old"],
        ...
        if "dp_monitor" in self.bc_dict.keys():
            ...
            self.bc.dp_monitor_bcs(
                self.bc_dict["dp_monitor"],
                self.a_u_,
                self.a_d_,
                self.pint_u_,
                self.pint_d_,
        ...
        if "membrane" in self.bc_dict.keys():
            self.mem_active_stress, self.mem_active_stress_type = (
                [False] * len(self.bc_dict["membrane"]),
                ["ode"] * len(self.bc_dict["membrane"]),
            )

            self.internalvars["tau_a"], self.internalvars_old["tau_a"], self.internalvars_mid["tau_a"] = (
                self.tau_a,
                self.tau_a_old,
            ...
            for nm in range(len(self.bc_dict["membrane"])):
                if "active_stress" in self.bc_dict["membrane"][nm]["params"].keys():
                    ...
                    try:
                        self.mem_active_stress_type[nm] = self.bc_dict["membrane"][nm]["params"]["active_stress"][
                            "type"
                        ]
                    except:
                        pass  # default is 'ode'
                    ...
                    if self.mem_active_stress_type[nm] == "ode":
                        ...
                        self.ti.funcs_to_update.append(
                            {
                                self.act_curve[-1]: self.ti.timecurves(
                                    self.bc_dict["membrane"][nm]["params"]["active_stress"]["activation_curve"]
                                )
                            }
                        )
                        ...
                        self.actstress.append(
                            activestress_activation(
                                self.bc_dict["membrane"][nm]["params"]["active_stress"],
                                self.act_curve[-1],
                                x_ref=self.x_ref,
                            )
                        )
                    elif (
                        self.mem_active_stress_type[nm] == "prescribed"
                    ):  # here we use act_curve as the prescibed stress directly
                        self.act_curve.append(fem.Function(self.Vd_scalar))
                        self.ti.funcs_to_update.append(
                            {
                                self.act_curve[-1]: self.ti.timecurves(
                                    self.bc_dict["membrane"][nm]["params"]["active_stress"]["prescribed_curve"]
                                )
                            }
                        )
                        self.ti.funcs_to_update_old.append(
                    ...
                    if "weight" in self.bc_dict["membrane"][nm]["params"]["active_stress"].keys():
                        ...
                        self.io.readfunction(
                            wact_func, self.bc_dict["membrane"][nm]["params"]["active_stress"]["weight"]
                        )
            ...
            w_membrane, self.idmem, self.bstress, self.bstrainenergy, self.bintpower = self.bc.membranesurf_bcs(
                ...
                wallfields=self.wallfields,
                actweights=self.actweights,
            )
        ...
        w_neumann_prestr, self.deltaW_prestr_kin, self.deltaW_prestr_int, self.deltaW_p_prestr = (
            ufl.as_ufl(0),
            ufl.as_ufl(0),
            ufl.as_ufl(0),
            [],
        ...
        if self.prestress_initial or self.prestress_initial_only:
            ...
            for n, M in enumerate(self.domain_ids):
                ...
                self.deltaW_p_prestr.append(
                    self.vf.deltaW_int_pres(self.v, self.var_p_[j], self.dx(M), F=self.alevar["Fale"])
                )
                ...
                if self.prestress_kinetic == "navierstokes_transient":
                    self.deltaW_prestr_kin += self.vf.deltaW_kin_navierstokes_transient(
                        self.acc_prestr, self.v, self.rho[n], self.dx(M), w=self.alevar["w"], F=self.alevar["Fale"]
                    )
                    ...
                    raise ValueError(
                        "Unknown prestress_kinetic option. Choose either 'navierstokes_transient', 'navierstokes_steady', 'stokes_transient', or 'none'."
                    )
            if "neumann_prestress" in self.bc_dict.keys():
                w_neumann_prestr = self.bc.neumann_prestress_bcs(
                    self.bc_dict["neumann_prestress"],
                    self.bmeasures,
            ...
            if "membrane" in self.bc_dict.keys():
                self.ufluid_prestr = (
                    self.v * self.prestress_dt
                )  # only incremental displacement needed, since MULF update actually yields a zero displacement after the step
                w_membrane_prestr, _, _, _, _ = self.bc.membranesurf_bcs(
                    self.bc_dict["membrane"],
                    self.ufluid_prestr,
                    self.v,
                    self.acc_prestr,
            ...
            self.deltaW_prestr_ext = (
                w_neumann_prestr + w_robin + w_stabneumann + w_stabneumann_mod + w_membrane_prestr + w_robin_valve
            )
        else:
        ...
        if self.stabilization is not None:
            ...
            try:
                vscale_vel_dep = self.stabilization["vscale_vel_dep"]
            except:
                vscale_vel_dep = False
            try:
                vscale_amp = self.stabilization["vscale_amp"]
            except:
                vscale_amp = 1.0
            ...
            try:
                symm = self.stabilization["symmetric"]
            except:
                symm = False
            ...
            try:
                red_scheme = self.stabilization["reduced_scheme"]
            except:
                red_scheme = False
            ...
            if self.stabilization["scheme"] == "supg_pspg":
                for n, M in enumerate(self.domain_ids):
                    ...
                    if self.fluid_governing_type == "navierstokes_transient":
                        if not red_scheme:
                            ...
                            residual_v_strong = self.vf.f_inert_strong_navierstokes_steady(
                                self.v, self.rho[n], w=self.alevar["w"], F=self.alevar["Fale"]
                            ) + self.vf.f_gradp_strong(self.p_[j], F=self.alevar["Fale"])
                            residual_v_strong_old = self.vf.f_inert_strong_navierstokes_steady(
                                self.v_old, self.rho[n], w=self.alevar["w_old"], F=self.alevar["Fale_old"]
                            ...
                            residual_v_strong_mid = self.vf.f_inert_strong_navierstokes_steady(
                                self.vel_mid, self.rho[n], w=self.alevar["w_mid"], F=self.alevar["Fale_mid"]
                        ...
                        if not red_scheme:
                            ...
                            residual_v_strong = self.vf.f_inert_strong_stokes_transient(
                                self.acc, self.v, self.rho[n], w=self.alevar["w"], F=self.alevar["Fale"]
                            ) + self.vf.f_gradp_strong(self.p_[j], F=self.alevar["Fale"])
                            residual_v_strong_old = self.vf.f_inert_strong_stokes_transient(
                                self.a_old, self.v_old, self.rho[n], w=self.alevar["w_old"], F=self.alevar["Fale_old"]
                            ) + self.vf.f_gradp_strong(self.p_old_[j], F=self.alevar["Fale_old"])
                    ...
                    self.deltaW_int += self.vf.stab_lsic(
                        self.v, tau_lsic, self.rho[n], self.dx(M), F=self.alevar["Fale"]
                    )
                    self.deltaW_int_old += self.vf.stab_lsic(
                        self.v_old, tau_lsic, self.rho[n], self.dx(M), F=self.alevar["Fale_old"]
                    )
                    self.deltaW_int_mid += self.vf.stab_lsic(
                        self.vel_mid, tau_lsic, self.rho[n], self.dx(M), F=self.alevar["Fale_mid"]
                    )
                    # PSPG (pressure-stabilizing Petrov-Galerkin) for Navier-Stokes and Stokes
                    self.deltaW_p[n] += self.vf.stab_pspg(
                        self.var_p_[j], residual_v_strong, tau_pspg, self.rho[n], self.dx_p[j](M), F=self.alevar["Fale"]
                    )
                    ...
                    if self.prestress_initial or self.prestress_initial_only:
                        ...
                        if self.prestress_kinetic == "navierstokes_transient":
                            ...
                            if not red_scheme:
                                ...
                                residual_v_strong_prestr = self.vf.f_inert_strong_stokes_transient(
                                    self.acc_prestr, self.v, self.rho[n], w=self.alevar["w"], F=self.alevar["Fale"]
                                ) + self.vf.f_gradp_strong(self.p_[j], F=self.alevar["Fale"])
                        elif self.prestress_kinetic == "none":
                            if not red_scheme:
                        ...
                        if (
                            self.prestress_kinetic == "navierstokes_transient"
            ...
            if self.stabilization is not None:
                for n in range(self.num_domains):
                    ...
                    self.weakform_lin_prestress_pp.append(
                        ufl.derivative(self.weakform_prestress_p[n], self.p_[j], self.dp_[j])
                    )
    ...
    def compute_strain_energy_power_membrane(self, N, t):
        ...
        for nm in range(len(self.bc_dict["membrane"])):
            try:
                internal = self.bc_dict["membrane"][nm]["internal"]
            except:
                internal = False

            if internal:
                try:
                    fcts = self.bc_dict["membrane"][nm]["facet_side"]
                except:
                    fcts = "+"
        ...
        if self.comm.rank == 0:
            if self.io.write_results_every > 0 and N % self.io.write_results_every == 0:
                ...
                if "strainenergy_membrane" in self.results_to_write:
                    fe = open(
                        self.io.output_path + "/results_" + self.pbase.simname + "_strainenergy_membrane.txt", mode
                    )
                    fe.write("%.16E %.16E\n" % (t, strain_energy_mem))
                    fe.close()
                if "internalpower_membrane" in self.results_to_write:
                    fp = open(
                        self.io.output_path + "/results_" + self.pbase.simname + "_internalpower_membrane.txt", mode
                    )
    ...
    def set_problem_vector_matrix_structures(self, rom=None):
        ...
        if self.have_robin_valve_implicit:
            ...
            self.K_zz = PETSc.Mat().createAIJ(
                size=(self.num_valve_coupling_surf, self.num_valve_coupling_surf),
                bsize=None,
                nnz=None,
                csr=None,
                comm=self.comm,
            ...
            for n in range(self.num_valve_coupling_surf):
                ...
                nds_v = np.array(
                    self.V_v.dofmap.index_map.local_to_global(np.asarray(nds_v_local, dtype=np.int32)), dtype=np.int32
                )
            ...
            self.K_vz = PETSc.Mat().createAIJ(
                size=((locmatsize, matsize), (PETSc.DECIDE, self.num_valve_coupling_surf)),
                bsize=None,
                nnz=self.num_valve_coupling_surf,
                csr=None,
                comm=self.comm,
            )
            self.K_vz.setUp()
    ...
    def assemble_residual(self, t, subsolver=None):
        ...
        with self.r_v.localForm() as r_local:
            r_local.set(0.0)
        ...
        if self.num_dupl > 1:
            fem.petsc.assemble_vector_block(
                self.r_p, self.res_p, self.dummat, bcs=[]
        ...
        if self.have_robin_valve_implicit:
            self.evaluate_dp_monitor(
                self.pu_, self.pd_, self.pint_u_, self.pint_d_, self.dp_, self.a_u_, self.a_d_, prnt=False
            )
    ...
    def assemble_stiffness(self, t, subsolver=None):
        ...
        if self.stabilization is not None:
            ...
            if self.num_dupl > 1:
                fem.petsc.assemble_matrix_block(
                    self.K_pp, self.jac_pp_, []
                )  # currently, we do not consider pressure DBCs
            else:
        ...
        if self.have_robin_valve_implicit:
            ...
            for i in range(len(self.col_ids)):
                with self.k_vz_vec[i].localForm() as r_local:
                    r_local.set(0.0)
                fem.petsc.assemble_vector(
                    self.k_vz_vec[i], self.dw_robin_valve_dz_form[i]
            ...
            for i in range(len(self.col_ids)):
                ...
                self.K_vz.setValues(
                    self.dofs_coupling_v[i], self.col_ids[i], self.k_vz_subvec[i].array, addv=PETSc.InsertMode.INSERT
                )
    ...
    def evaluate_robin_valve(self, t, dp_):
        ...
        for m in range(len(self.bc_dict["robin_valve"])):
            ...
            try:
                self.beta_valve_max_expr[m].t = t
            except:
                pass

            try:
                self.beta_valve_min_expr[m].t = t
            except:
                pass
            ...
            if self.bc_dict["robin_valve"][m]["type"] == "dp":
                ...
                beta.val = (
                    0.5
                    * (self.beta_valve_max_expr[m].val - self.beta_valve_min_expr[m].val)
                    * (ufl.tanh(dp_[dp_id] / epsilon) + 1.0)
                    + self.beta_valve_min_expr[m].val
                )
    ...
    def evaluate_dp_monitor(self, pu_, pd_, pint_u_, pint_d_, dp_, a_u_, a_d_, prnt=True):
        for m in range(len(self.bc_dict["dp_monitor"])):
            ...
            pu = (1.0 / au_) * fem.assemble_scalar(pint_u_[m])
            pu = self.comm.allgather(pu)
            pu_[m] = sum(pu)

            pd = (1.0 / ad_) * fem.assemble_scalar(pint_d_[m])
            ...
            if prnt:
                utilities.print_status(
                    "dp ID " + str(self.bc_dict["dp_monitor"][m]["id"]) + ": pu = %.4e, pd = %.4e" % (pu_[m], pd_[m]),
                    self.comm,
                )
    ...
    def evaluate_flux_monitor(self, qv_, q_, prnt=True):
        for m in range(len(self.bc_dict["flux_monitor"])):
            ...
            if prnt:
                utilities.print_status(
                    "Flux ID " + str(self.bc_dict["flux_monitor"][m]["id"]) + ": q = %.4e" % (qv_[m]), self.comm
                )
    ...
    def evaluate_initial(self):
        if self.have_flux_monitor:
            ...
            for k in self.qv_old_:
                self.qv_[k] = self.qv_old_[k]
        if self.have_dp_monitor:
            ...
            for k in self.pu_old_:
                self.pu_[k] = self.pu_old_[k]
            for k in self.pd_old_:
                self.pd_[k] = self.pd_old_[k]
        if self.have_robin_valve:
    ...
    def update(self):
        ...
        if self.have_flux_monitor:
            for k in self.qv_:
                self.qv_old_[k] = self.qv_[k]
        if self.have_dp_monitor:
            for k in self.pu_:
                self.pu_old_[k] = self.pu_[k]
            for k in self.pd_:
                self.pd_old_[k] = self.pd_[k]
...
class FluidmechanicsSolver(solver_base):
    ...
    def solve_initial_state(self):
        ...
        if (
            self.pb.fluid_governing_type == "navierstokes_transient"
            or self.pb.fluid_governing_type == "stokes_transient"
        ) and self.pb.pbase.restart_step == 0:
            ts = time.time()
            utilities.print_status(
                "Setting forms and solving for consistent initial acceleration...", self.pb.comm, e=" "
            )
    ...
    def solve_initial_prestress(self):
        ...
        for N in range(1, self.pb.prestress_numstep + 1):
            ...
            self.pb.ti.print_prestress_step(
                N, tprestr, self.pb.prestress_numstep, self.solnln.lsp, ni=self.solnln.ni, li=self.solnln.li, wt=wt
            )
        ...
        if self.pb.prestress_initial_only:
            ...
            self.pb.io.writefunction(
                self.pb.uf_pre,
                self.pb.io.output_path_pre + "/results_" + self.pb.pbase.simname + "_fluiddisplacement_pre",
            )
            if bool(self.pb.io.duplicate_mesh_domains):
                ...
                self.pb.io.writefunction(
                    self.pb.p_[0], self.pb.io.output_path_pre + "/results_" + self.pb.pbase.simname + "_pressure_pre"
                )
            utilities.print_status(
                "Prestress only done. To resume, set file path(s) in 'prestress_from_file' and read in uf_pre.",
                self.pb.comm,
            )
            os._exit(0)
        ...
        if self.pb.prestress_ptc:
            try:
                self.solnln.PTC = self.solver_params["ptc"]
            except:
                self.solnln.PTC = False
