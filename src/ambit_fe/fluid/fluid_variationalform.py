...
class variationalform(variationalform_base):
    ...
    def stab_supg(self, v, res_v_strong, tau_supg, rho, ddomain, w=None, F=None, symmetric=False):
        if symmetric:  # modification to make the effective stress symmetric - experimental, use with care...
            return (1.0 / rho) * ufl.dot(tau_supg * rho * ufl.sym(ufl.grad(self.var_v)) * v, res_v_strong) * ddomain
        else:
...
class variationalform_ale(variationalform):
    ...
    def deltaW_kin_navierstokes_transient(self, a, v, rho, ddomain, w=None, F=None):
        ...
        if self.formulation == "nonconservative":
            ...
            return (
                rho
                * ufl.dot(a + ufl.as_vector(ufl.grad(ufl.outer(v, v - w))[i, j, k] * ufl.inv(F).T[j, k], i), self.var_v)
                * J
                * ddomain
            )
    ...
    def deltaW_kin_navierstokes_steady(self, v, rho, ddomain, w=None, F=None):
        ...
        if self.formulation == "nonconservative":
            ...
            return (
                rho
                * ufl.dot(ufl.as_vector(ufl.grad(ufl.outer(v, v - w))[i, j, k] * ufl.inv(F).T[j, k], i), self.var_v)
                * J
                * ddomain
            )
        else:
    ...
    def deltaW_kin_stokes_transient(self, a, v, rho, ddomain, w=None, F=None):
        ...
        if self.formulation == "nonconservative":
            ...
            return (
                rho
                * ufl.dot(a + ufl.as_vector(ufl.grad(ufl.outer(v, -w))[i, j, k] * ufl.inv(F).T[j, k], i), self.var_v)
                * J
                * ddomain
            )
        else:
    ...
    def stab_supg(self, v, res_v_strong, tau_supg, rho, ddomain, w=None, F=None, symmetric=False):
        ...
        if symmetric:  # modification to make the effective stress symmetric - experimental, use with care...
            return (
                (1.0 / rho)
                * ufl.dot(tau_supg * rho * ufl.sym(ufl.grad(self.var_v) * ufl.inv(F)) * v, res_v_strong)
                * J
                * ddomain
            )
        else:
            return (
                (1.0 / rho)
                * ufl.dot(tau_supg * rho * ufl.grad(self.var_v) * ufl.inv(F) * v, res_v_strong)
                * J
                * ddomain
            )
