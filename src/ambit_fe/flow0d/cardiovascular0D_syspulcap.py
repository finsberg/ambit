...
class cardiovascular0Dsyspulcap(cardiovascular0Dbase):
    def __init__(
        ...
        try:
            ...
            self.R_arcor_sys = params["R_corp_sys"]
        try:
            self.C_arcor_sys = params["C_arcor_sys"]
        except:
            self.C_arcor_sys = params["C_corp_sys"]
        ...
        try:
            ...
            self.R_vencor_sys = params["R_cord_sys"]
        try:
            self.C_vencor_sys = params["C_vencor_sys"]
        except:
            self.C_vencor_sys = params["C_cord_sys"]
        ...
        self.V_arspl_sys_u = params.get("V_arspl_sys_u", 0.0)
        self.V_arespl_sys_u = params.get("V_arespl_sys_u", 0.0)
        self.V_armsc_sys_u = params.get("V_armsc_sys_u", 0.0)
        self.V_arcer_sys_u = params.get("V_arcer_sys_u", 0.0)
        self.V_arcor_sys_u = params.get("V_arcor_sys_u", 0.0)
        self.V_venspl_sys_u = params.get("V_venspl_sys_u", 0.0)
        self.V_venespl_sys_u = params.get("V_venespl_sys_u", 0.0)
        self.V_venmsc_sys_u = params.get("V_venmsc_sys_u", 0.0)
        self.V_vencer_sys_u = params.get("V_vencer_sys_u", 0.0)
        self.V_vencor_sys_u = params.get("V_vencor_sys_u", 0.0)
        self.V_ven_sys_u = params.get("V_ven_sys_u", 0.0)
        self.V_ar_pul_u = params.get("V_ar_pul_u", 0.0)
        self.V_cap_pul_u = params.get("V_cap_pul_u", 0.0)
    ...
    def equation_map(self):
        ...
        vl_mv_, R_vin_l_ = self.valvelaw(
            p_at_l_o1_, p_v_l_i1_, self.R_vin_l_min, self.R_vin_l_max, self.valvelaws["mv"], self.t_es, self.t_ed
        )
        vl_av_, R_vout_l_ = self.valvelaw(
            p_v_l_o1_, p_ar_sys_, self.R_vout_l_min, self.R_vout_l_max, self.valvelaws["av"], self.t_ed, self.t_es
        )
        vl_tv_, R_vin_r_ = self.valvelaw(
            p_at_r_o1_, p_v_r_i1_, self.R_vin_r_min, self.R_vin_r_max, self.valvelaws["tv"], self.t_es, self.t_ed
        )
        vl_pv_, R_vout_r_ = self.valvelaw(
            p_v_r_o1_, p_ar_pul_, self.R_vout_r_min, self.R_vout_r_max, self.valvelaws["pv"], self.t_ed, self.t_es
        )
        ...
        self.df_[8] = (
            self.C_arspl_sys + self.C_arespl_sys + self.C_armsc_sys + self.C_arcer_sys + self.C_arcor_sys
        ) * p_arperi_sys_  # systemic capillary volume rate
        self.df_[9] = 0.0
        self.df_[10] = 0.0
        self.df_[11] = 0.0
        self.df_[12] = 0.0
        ...
        self.f_[8] = -q_ar_sys_ + (
            q_arspl_sys_ + q_arespl_sys_ + q_armsc_sys_ + q_arcer_sys_ + q_arcor_sys_
        )  # systemic capillary flow balance
        self.f_[9] = (
            p_venspl_sys_ - p_arperi_sys_
        ) / self.R_arspl_sys + q_arspl_sys_  # systemic arterial splanchnic momentum
        self.f_[10] = (
            p_venespl_sys_ - p_arperi_sys_
        ) / self.R_arespl_sys + q_arespl_sys_  # systemic arterial extra-splanchnic momentum
        self.f_[11] = (
            p_venmsc_sys_ - p_arperi_sys_
        ) / self.R_armsc_sys + q_armsc_sys_  # systemic arterial muscular momentum
        self.f_[12] = (
            p_vencer_sys_ - p_arperi_sys_
        ) / self.R_arcer_sys + q_arcer_sys_  # systemic arterial cerebral momentum
        self.f_[13] = (
            p_vencor_sys_ - p_arperi_sys_
        ) / self.R_arcor_sys + q_arcor_sys_  # systemic arterial coronary momentum
        self.f_[14] = q_venspl_sys_ - q_arspl_sys_  # systemic venous splanchnic flow balance
        self.f_[15] = (
            p_ven_sys_ - p_venspl_sys_
        ) / self.R_venspl_sys + q_venspl_sys_  # systemic venous splanchnic momentum
        self.f_[16] = q_venespl_sys_ - q_arespl_sys_  # systemic venous extra-splanchnic flow balance
        self.f_[17] = (
            p_ven_sys_ - p_venespl_sys_
        ) / self.R_venespl_sys + q_venespl_sys_  # systemic venous extra-splanchnic momentum
        self.f_[18] = q_venmsc_sys_ - q_armsc_sys_  # systemic venous muscular flow balance
        self.f_[19] = (
            p_ven_sys_ - p_venmsc_sys_
        ) / self.R_venmsc_sys + q_venmsc_sys_  # systemic venous muscular momentum
        self.f_[20] = q_vencer_sys_ - q_arcer_sys_  # systemic venous cerebral flow balance
        self.f_[21] = (
            p_ven_sys_ - p_vencer_sys_
        ) / self.R_vencer_sys + q_vencer_sys_  # systemic venous cerebral momentum
        self.f_[22] = q_vencor_sys_ - q_arcor_sys_  # systemic venous coronary flow balance
        self.f_[23] = (
            p_ven_sys_ - p_vencor_sys_
        ) / self.R_vencor_sys + q_vencor_sys_  # systemic venous coronary momentum
        self.f_[24] = sum(q_ven_sys_) - (
            q_venspl_sys_ + q_venespl_sys_ + q_venmsc_sys_ + q_vencer_sys_ + q_vencor_sys_
        )  # systemic venous flow balance
        for n in range(self.vs):
            self.f_[25 + n] = (p_at_r_i_[n] - p_ven_sys_) / R_ven_sys[n] + q_ven_sys_[n]  # systemic venous momentum
            # -----------------------------------------------------------
        self.f_[25 + self.vs] = (
            -sum(q_ven_sys_) + q_vin_r_ - (1 - self.switch_V[3]) * VQ_at_r_
        ...
        for n in range(self.vp):
            self.f_[34 + self.vs + n] = (p_at_l_i_[n] - p_ven_pul_) / R_ven_pul[n] + q_ven_pul_[
                n
        ...
        if check[0] == "allvar":
            ...
            var_ids, aux_ids = (
                [
                    ...
                    self.varmap["p_ar_sys"],
                    self.varmap["p_ard_sys"],
                    self.varmap["p_arperi_sys"],
                    self.varmap["p_venspl_sys"],
                    self.varmap["p_venespl_sys"],
                    self.varmap["p_venmsc_sys"],
                    self.varmap["p_vencer_sys"],
                    self.varmap["p_vencor_sys"],
                    self.varmap["p_ven_sys"],
                    self.varmap[self.vname[3]],
                    self.varmap[self.vname[1]],
                    self.varmap["p_ar_pul"],
                    self.varmap["p_cap_pul"],
    ...
    def print_to_screen(self, var, aux):
        ...
        utilities.print_status("Output of 0D vascular model (syspulcap):", self.comm)

        for i in range(nc):
            utilities.print_status(
                "{:<12s}{:<3s}{:<10.3f}".format(self.cname[i], " = ", aux[self.auxmap[self.cname[i]]]), self.comm
            )

        utilities.print_status(
            "{:<12s}{:<3s}{:<10.3f}{:<3s}{:<9s}{:<3s}{:<10.3f}".format(
                self.vname[2],
                " = ",
        ...
        utilities.print_status(
            "{:<12s}{:<3s}{:<10.3f}{:<3s}{:<9s}{:<3s}{:<10.3f}".format(
                "p_arperi_sys",
                " = ",
                var_arr[self.varmap["p_arperi_sys"]],
                "   ",
                "p_cap_pul",
                " = ",
                var_arr[self.varmap["p_cap_pul"]],
            ),
            self.comm,
        )
        utilities.print_status(
...
class cardiovascular0Dsyspulcapcor(cardiovascular0Dsyspulcap):
    ...
    def equation_map(self):
        ...
        if (
            self.cormodel is not None
        ):  # p_v_l on purpose here NOT passed to coronary model (for comparison reasons to syspulcap model...)
        ...
        self.df_[7] = (self.L_ar_sys / self.R_ar_sys) * q_ar_sys_  # systemic arterial inertia
        self.df_[8] = (
            self.C_arspl_sys + self.C_arespl_sys + self.C_armsc_sys + self.C_arcer_sys
        ) * p_arperi_sys_  # systemic capillary volume rate
        self.df_[9] = 0.0
        self.df_[10] = 0.0
        self.df_[11] = 0.0
        ...
        self.f_[7] = (p_arperi_sys_ - p_ard_sys_) / self.R_ar_sys + q_ar_sys_  # systemic arterial momentum
        self.f_[8] = -q_ar_sys_ + (
            q_arspl_sys_ + q_arespl_sys_ + q_armsc_sys_ + q_arcer_sys_
        )  # systemic capillary flow balance
        self.f_[9] = (
            p_venspl_sys_ - p_arperi_sys_
        ) / self.R_arspl_sys + q_arspl_sys_  # systemic arterial splanchnic momentum
        ...
        self.f_[21] = sum(q_ven_sys_) - (
            q_venspl_sys_ + q_venespl_sys_ + q_venmsc_sys_ + q_vencer_sys_
        )  # systemic venous flow balance
        for n in range(self.vs):
        ...
        )  # right atrium flow balance: all veins incl. coronary sinus (if present)
        ...
        for n in range(self.vp):
            self.f_[31 + self.vs + n] = (p_at_l_i_[n] - p_ven_pul_) / R_ven_pul[n] + q_ven_pul_[
                n
            ]  # pulmonary venous momentum
        ...
        self.a_[nc + 4] = self.C_ar_sys * p_ard_sys_ + self.V_ar_sys_u
        self.a_[nc + 5] = (
            (self.C_arspl_sys + self.C_arespl_sys + self.C_armsc_sys + self.C_arcer_sys) * p_arperi_sys_
            + self.V_arspl_sys_u
            + self.V_arespl_sys_u
            + self.V_armsc_sys_u
            + self.V_arcer_sys_u
    ...
    def print_to_screen(self, var, aux):
        ...
        utilities.print_status("Output of 0D vascular model (syspulcapcor):", self.comm)
