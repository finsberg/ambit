...
class cardiovascular0Dbase(ode):
    ...
    def cycle_check(
        ...
        if self.T_cycl > 0.0 and np.isclose(math.fmod(t, self.T_cycl), 0.0):
            ...
            if check is not None:
                is_periodic = self.check_periodic(varTc, varTc_old, auxTc, auxTc_old, eps_periodic, check, cyclerr)
    ...
    def induce_perturbation(self, perturb_type, perturb_factor):
        if perturb_type == "mr":
            self.R_vin_l_max *= perturb_factor
        if perturb_type == "ms":
            self.R_vin_l_min *= perturb_factor
        if perturb_type == "ar":
            self.R_vout_l_max *= perturb_factor
        if perturb_type == "as":
            self.R_vout_l_min *= perturb_factor
    ...
    def valvelaw(self, p, popen, Rmin, Rmax, vparams, topen, tclose):
        if vparams[0] == "pwlin_pres":  # piecewise linear with resistance depending on pressure difference
            ...
            R = 0.5 * (Rmax - Rmin) * (sp.tanh((popen - p) / vparams[-1]) + 1.0) + Rmin
            vl = (popen - p) / R
        elif vparams[0] == "smooth_pres_momentum":  # smooth q(p) relationship
            # interpolation by cubic spline in epsilon interval
            p0 = (popen - vparams[-1] / 2.0 - popen) / Rmax
            p1 = (popen + vparams[-1] / 2.0 - popen) / Rmin
            m0 = 1.0 / Rmax
            m1 = 1.0 / Rmin
            s = (p - (popen - vparams[-1] / 2.0)) / vparams[-1]
            # spline ansatz functions
            h00 = 2.0 * s**3.0 - 3 * s**2.0 + 1.0
            h01 = -2.0 * s**3.0 + 3 * s**2.0
            h10 = s**3.0 - 2.0 * s**2.0 + s
            h11 = s**3.0 - s**2.0
            # spline
            c = h00 * p0 + h10 * m0 * vparams[-1] + h01 * p1 + h11 * m1 * vparams[-1]
            vl = sp.Piecewise(
                ((popen - p) / Rmax, p < popen - vparams[-1] / 2),
                (-c, sp.And(p >= popen - vparams[-1] / 2.0, p < popen + vparams[-1] / 2.0)),
                ((popen - p) / Rmin, p >= popen + vparams[-1] / 2.0),
            )
        elif vparams[0] == "pw_pres_regurg":
            vl = sp.Piecewise(
                (vparams[1] * vparams[2] * sp.sqrt(popen - p), p < popen), ((popen - p) / Rmin, p >= popen)
            )
        else:
    ...
    def set_compartment_interfaces(self):
        ...
        for i, ch in enumerate(["lv", "rv", "la", "ra", "ao"]):
            try:
                num_infl[i] = self.chmodels[ch]["num_inflows"]
            except:
                num_infl[i] = 1
            try:
                num_outfl[i] = self.chmodels[ch]["num_outflows"]  # actually not used so far...
            except:
                num_outfl[i] = 1
        ...
        for i, ch in enumerate(["lv", "rv", "la", "ra", "ao"]):
            ...
            if ch == "ao":  # allow 1 in-flow, 3 ouf-flows for now...
                ...
                if self.cormodel:
                    ind_o = [
                        16 + num_infl[3] + num_infl[2],
                        20 + num_infl[3] + num_infl[2],
                        5,
                    ]  # q_corp_sys_l_in, q_corp_sys_r_in, q_arp_sys
                else:
                    ind_o = [5]  # q_arp_sys
    ...
    def set_coupling_state(self, ch, chvars, chfncs=[]):
        if ch == "lv":
            V_unstressed, i = self.V_v_l_u, 0
        if ch == "rv":
            V_unstressed, i = self.V_v_r_u, 1
        if ch == "la":
            V_unstressed, i = self.V_at_l_u, 2
        if ch == "ra":
            V_unstressed, i = self.V_at_r_u, 3
        if ch == "ao":
            V_unstressed, i = self.V_ar_sys_u, 4
        ...
        if self.chmodels[ch]["type"] == "0D_elast" or self.chmodels[ch]["type"] == "0D_elast_prescr":
            ...
            for k in range(10):  # no more than 10 distributed p's allowed
                if "pi" + str(k + 1) in chvars.keys():
                    chvars["pi" + str(k + 1)] = chvars["pi1"]
                if "po" + str(k + 1) in chvars.keys():
                    chvars["po" + str(k + 1)] = chvars["pi1"]
        ...
        # 3D fluid mechanics model
            ...
            if self.chmodels[ch]["num_inflows"] == 0:
                chvars["pi1"] = sp.S.Zero
    ...
    def evaluate_chamber_state(self, y, t):
        ...
        for i, ch in enumerate(["lv", "rv", "la", "ra"]):
            if self.chmodels[ch]["type"] == "0D_elast":
                if ch == "lv":
                    E_max, E_min = self.E_v_max_l, self.E_v_min_l
                if ch == "rv":
                    E_max, E_min = self.E_v_max_r, self.E_v_min_r
                if ch == "la":
                    E_max, E_min = self.E_at_max_l, self.E_at_min_l
                if ch == "ra":
                    E_max, E_min = self.E_at_max_r, self.E_at_min_r
    ...
    def initialize_lm(self, var, iniparam):
        ...
        for ch in ["lv", "rv", "la", "ra", "ao"]:
            ...
            if self.chmodels[ch]["type"] == "3D_solid":
                if "p_" + chn + "_0" in iniparam.keys():
                    var[ci] = iniparam["p_" + chn + "_0"]
                ci += 1

            if self.chmodels[ch]["type"] == "3D_fluid":
                # in-flow pressures
                for m in range(self.chmodels[ch]["num_inflows"]):
                    if "p_" + chn + "_i" + str(m + 1) in iniparam.keys():
                        var[ci] = iniparam["p_" + chn + "_i" + str(m + 1)]
                    ci += 1

                # out-flow pressures
                for m in range(self.chmodels[ch]["num_outflows"]):
                    if "p_" + chn + "_o" + str(m + 1) in iniparam.keys():
                        var[ci] = iniparam["p_" + chn + "_o" + str(m + 1)]
                    ci += 1
