...
class coronary_circ_ZCRp_CRd:
    def __init__(self, params, varmap, auxmap, vs):
        ...
        try:
            self.V_corp_sys_u = params["V_corp_sys_u"]
        except:
            self.V_corp_sys_u = 0
        try:
            self.V_cord_sys_u = params["V_cord_sys_u"]
        except:
            self.V_cord_sys_u = 0
    ...
    def equation_map(self, vindex, aindex, x_, a_, df_, f_, p_ar_, p_v_, p_at_):
        ...
        f_[vindex + 1] = (
            p_cord_sys_ - p_ar_[0] + self.Z_corp_sys * q_corp_sys_in_
        ) / self.R_corp_sys + q_corp_sys_  # coronary proximal momentum
        ...
        a_[self.auxmap["V_corp_sys"]] = (
            self.C_corp_sys * (p_ar_[0] - self.Z_corp_sys * q_corp_sys_in_) + self.V_corp_sys_u
        )
        ...
        if p_v_ is sp.S.Zero:
            raise ValueError("Zero symbol for left ventricular pressure!")

        return [q_corp_sys_in_], q_cord_sys_

    def initialize(self, var, iniparam):
        try:
            var[self.varmap["q_corp_sys_in"]] = iniparam["q_corp_sys_in_0"]
        except:
            var[self.varmap["q_corp_sys_in"]] = iniparam["q_corp_sys_0"]
        var[self.varmap["q_corp_sys"]] = iniparam["q_corp_sys_0"]
        var[self.varmap["p_cord_sys"]] = iniparam["p_cord_sys_0"]

        try:
            var[self.varmap["q_ven" + str(self.vs + 1) + "_sys"]] = iniparam["q_ven" + str(self.vs + 1) + "_sys_0"]
        except:
            var[self.varmap["q_ven" + str(self.vs + 1) + "_sys"]] = iniparam["q_ven_sys_0"]
    ...
    def print_to_screen(self, var_arr, aux, comm):
        ...
        utilities.print_status(
            "{:<10s}{:<3s}{:<7.3f}".format("p_cord_sys", " = ", var_arr[self.varmap["p_cord_sys"]]), comm
        )
...
class coronary_circ_ZCRp_CRd_lr:
    def __init__(self, params, varmap, auxmap, vs):
        ...
        try:
            self.V_corp_sys_l_u = params["V_corp_sys_l_u"]
        except:
            self.V_corp_sys_l_u = 0
        try:
            self.V_cord_sys_l_u = params["V_cord_sys_l_u"]
        except:
            self.V_cord_sys_l_u = 0

        try:
            self.V_corp_sys_r_u = params["V_corp_sys_r_u"]
        except:
            self.V_corp_sys_r_u = 0
        try:
            self.V_cord_sys_r_u = params["V_cord_sys_r_u"]
        except:
            self.V_cord_sys_r_u = 0
    ...
    def equation_map(self, vindex, aindex, x_, a_, df_, f_, p_ar_, p_v_, p_at_):
        ...
        # populate df_ and f_ arrays
        df_[vindex] = self.C_corp_sys_l * (
            p_ar_[0] - self.Z_corp_sys_l * q_corp_sys_l_in_
        )  # left coronary proximal volume rate
        df_[vindex + 1] = 0.0
        df_[vindex + 2] = self.C_cord_sys_l * (p_cord_sys_l_ - p_v_)  # left coronary distal volume rate
        df_[vindex + 3] = 0.0
        df_[vindex + 4] = self.C_corp_sys_r * (
            p_ar_[1] - self.Z_corp_sys_r * q_corp_sys_r_in_
        )  # right coronary proximal volume rate
        df_[vindex + 5] = 0.0
        ...
        f_[vindex + 1] = (
            p_cord_sys_l_ - p_ar_[0] + self.Z_corp_sys_l * q_corp_sys_l_in_
        ) / self.R_corp_sys_l + q_corp_sys_l_  # left coronary proximal momentum
        ...
        f_[vindex + 5] = (
            p_cord_sys_r_ - p_ar_[1] + self.Z_corp_sys_r * q_corp_sys_r_in_
        ) / self.R_corp_sys_r + q_corp_sys_r_  # right coronary proximal momentum
        ...
        a_[self.auxmap["V_corp_sys_l"]] = (
            self.C_corp_sys_l * (p_ar_[0] - self.Z_corp_sys_l * q_corp_sys_l_in_) + self.V_corp_sys_l_u
        )
        a_[self.auxmap["V_cord_sys_l"]] = self.C_cord_sys_l * (p_cord_sys_l_ - p_v_) + self.V_cord_sys_l_u
        a_[self.auxmap["V_corp_sys_r"]] = (
            self.C_corp_sys_r * (p_ar_[1] - self.Z_corp_sys_r * q_corp_sys_r_in_) + self.V_corp_sys_r_u
        )
    ...
    def initialize(self, var, iniparam):
        try:
            var[self.varmap["q_corp_sys_l_in"]] = iniparam["q_corp_sys_l_in_0"]
        except:
            var[self.varmap["q_corp_sys_l_in"]] = iniparam["q_corp_sys_l_0"]
        ...
        try:
            var[self.varmap["q_corp_sys_r_in"]] = iniparam["q_corp_sys_r_in_0"]
        except:
            var[self.varmap["q_corp_sys_r_in"]] = iniparam["q_corp_sys_r_0"]
    ...
    def print_to_screen(self, var_arr, aux, comm):
        ...
        utilities.print_status(
            "{:<12s}{:<3s}{:<7.3f}{:<3s}{:<12s}{:<3s}{:<7.3f}".format(
                "p_cord_sys_l",
                " = ",
                var_arr[self.varmap["p_cord_sys_l"]],
                "   ",
                "p_cord_sys_r",
                " = ",
                var_arr[self.varmap["p_cord_sys_r"]],
            ),
            comm,
        )
