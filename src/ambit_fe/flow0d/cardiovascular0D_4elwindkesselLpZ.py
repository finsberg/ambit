...
class cardiovascular0D4elwindkesselLpZ(cardiovascular0Dbase):
    def __init__(self, params, cq, vq, init=True, ode_par=False, comm=None):
        ...
        for n in range(self.num_models):
            # resistance
            try:
                self.R.append(params["R" + str(n + 1)])
            except:
                self.R.append(params["R"])
            # compliance
            try:
                self.C.append(params["C" + str(n + 1)])
            except:
                self.C.append(params["C"])
            # impedance
            try:
                self.Z.append(params["Z" + str(n + 1)])
            except:
                self.Z.append(params["Z"])
            # inertance
            try:
                self.L.append(params["L" + str(n + 1)])
            except:
                self.L.append(params["L"])
            # downstream reference pressure
            try:
                self.p_ref.append(params["p_ref" + str(n + 1)])
            except:
                self.p_ref.append(params["p_ref"])
    ...
    def equation_map(self):
        ...
        for n in range(self.num_models):
            ...
            self.f_[4 * n + 0] = (
                (self.L[n] / (self.R[n] * self.Z[n]) + self.C[n]) * g_[n]
                + (p_[n] - self.p_ref[n]) / self.R[n]
                + q_[n]
                + (self.L[n] / self.R[n] + self.L[n] / self.Z[n]) * s_[n]
            )
            self.f_[4 * n + 1] = -g_[n]
    ...
    def initialize(self, var, iniparam):
        for n in range(self.num_models):
            try:
                var[4 * n + 0] = iniparam[self.vname[n] + str(n + 1) + "_0"]
            except:
                var[4 * n + 0] = iniparam[self.vname[n] + "_0"]
            try:
                var[4 * n + 1] = iniparam["g" + str(n + 1) + "_0"]
            except:
                var[4 * n + 1] = iniparam["g_0"]
            try:
                var[4 * n + 2] = iniparam["q" + str(n + 1) + "_0"]
            except:
                var[4 * n + 2] = iniparam["q_0"]
            try:
                var[4 * n + 3] = iniparam["s" + str(n + 1) + "_0"]
            except:
                var[4 * n + 3] = iniparam["s_0"]

    def initialize_lm(self, var, iniparam):
        for n in range(self.num_models):
            if "p_0" in iniparam.keys():
                var[n] = iniparam["p_0"]
            if "p" + str(n + 1) + "_0" in iniparam.keys():
                var[n] = iniparam["p" + str(n + 1) + "_0"]
