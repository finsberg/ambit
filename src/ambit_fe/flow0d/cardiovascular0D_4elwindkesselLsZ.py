...
class cardiovascular0D4elwindkesselLsZ(cardiovascular0Dbase):
    ...
    def equation_map(self):
        ...
        for n in range(self.num_models):
            ...
            self.f_[3 * n + 0] = (
                ...
                + (1.0 + self.Z[n] / self.R[n]) * q_[n]
                + (self.C[n] * self.Z[n] + self.L[n] / self.R[n]) * s_[n]
            )
    ...
    def initialize(self, var, iniparam):
        for n in range(self.num_models):
            try:
                var[3 * n + 0] = iniparam[self.vname[n] + str(n + 1) + "_0"]
            except:
                var[3 * n + 0] = iniparam[self.vname[n] + "_0"]
            try:
                var[3 * n + 1] = iniparam["q" + str(n + 1) + "_0"]
            except:
                var[3 * n + 1] = iniparam["q_0"]
            try:
                var[3 * n + 2] = iniparam["s" + str(n + 1) + "_0"]
            except:
                var[3 * n + 2] = iniparam["s_0"]
