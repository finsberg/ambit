...
class cardiovascular0DCRLinoutlink(cardiovascular0Dbase):
    ...
    def initialize_lm(self, var, iniparam):
        if "p_in_0" in iniparam.keys():
            var[0] = iniparam["p_in_0"]
        if "p_out_0" in iniparam.keys():
            var[1] = iniparam["p_out_0"]
    ...
    def print_to_screen(self, var, aux):
        ...
        for i in range(len(self.cname)):
            utilities.print_status(
                "{:<5s}{:<3s}{:<10.3f}".format(self.cname[i], " = ", aux[self.auxmap[self.cname[i]]]), self.comm
            )
        for i in range(len(self.vname)):
            utilities.print_status(
                "{:<5s}{:<3s}{:<10.3f}".format(self.vname[i], " = ", var_arr[self.varmap[self.vname[i]]]), self.comm
            )
