...
class cardiovascular0Dsyspul(cardiovascular0Dbase):
    ...
    def equation_map(self):
        ...
        # aortic root/ascending aortic compartment
        VQ_aort_sys_, p_ar_sys_i1_, p_ar_sys_o1_, p_ar_sys_o2_, p_ar_sys_o3_, p_ar_sys_o4_ = (
            chdict_ao["VQ"],
            chdict_ao["pi1"],
            chdict_ao["po1"],
            chdict_ao["po2"],
        ...
        if self.vadmodel is not None:
            q_vad_in_, q_vad_out_ = self.vadcirc.equation_map(
                self.doff_vad, len(self.c_) + 8, self.x_, self.a_, self.df_, self.f_, p_v_l_o2_, p_ar_sys_o4_
            )
        else:
        ...
        vl_av_, R_vout_l_ = self.valvelaw(
            p_v_l_o1_, p_ar_sys_i1_, self.R_vout_l_min, self.R_vout_l_max, self.valvelaws["av"], self.t_ed, self.t_es
        )
    ...
    def print_to_screen(self, var, aux):
        ...
        for i in range(nc):
            utilities.print_status(
                "{:<9s}{:<3s}{:<10.3f}".format(self.cname[i], " = ", aux[self.auxmap[self.cname[i]]]), self.comm
            )
