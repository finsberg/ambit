...
class Flow0DProblem(problem_base):
    def __init__(self, pbase, io_params, time_params, model_params, time_curves, coupling_params={}):
        ...
        try:
            self.coronary_model = model_params["coronary_model"]
        except:
            self.coronary_model = None

        try:
            self.vad_model = model_params["vad_model"]
        except:
            self.vad_model = None
        ...
        try:
            self.output_midpoint = io_params["output_midpoint_0D"]
        except:
            self.output_midpoint = False
        ...
        try:
            self.vq = coupling_params["variable_quantity"]
        except:
            self.vq = ["pressure"] * 5

        try:
            self.coup_type = coupling_params["coupling_type"]
        except:
            self.coup_type = None
        ...
        try:
            self.initial_backwardeuler = time_params["initial_backwardeuler"]
        except:
            self.initial_backwardeuler = False

        try:
            self.ode_parallel = io_params["ode_parallel"]
        except:
            self.ode_parallel = False
        ...
        if model_params["modeltype"] == "2elwindkessel":
            ...
            self.cardvasc0D = cardiovascular0D2elwindkessel(
                model_params["parameters"], self.cq, self.vq, init=True, ode_par=self.ode_parallel, comm=self.comm
            )
        ...
        if self.ode_parallel:
            self.K = PETSc.Mat().createAIJ(
                size=(self.numdof, self.numdof), bsize=None, nnz=None, csr=None, comm=self.comm
            )
        else:
        ...
        self.ti = timeintegration.timeintegration_flow0d(
            time_params, self.pbase.dt, self.pbase.numstep, time_curves, self.pbase.t_init, comm=self.comm
        )
    ...
    def readrestart(self, sname, rst, ms=False):
        ...
        if self.cardvasc0D.T_cycl > 0:  # read heart cycle info
            self.ti.cycle[0] = np.loadtxt(
                self.output_path_0D + "/checkpoint_" + sname + "_cycledata_" + str(rst) + ".txt", usecols=(0), dtype=int
            )
        ...
        if bool(self.auxdata_old):
            if bool(self.auxdata_old["p"]):
                auxdata_p = np.loadtxt(
                    self.output_path_0D + "/checkpoint_" + sname + "_auxdata_old_p_" + str(rst) + ".txt", ndmin=1
                )
    ...
    def evaluate_initial(self):
        ...
        self.cardvasc0D.evaluate(
            self.s_old, self.pbase.t_init, self.df_old, self.f_old, None, None, self.c, self.y, self.aux_old
        )
    ...
    def set_output_state(self, t):
        ...
        self.cardvasc0D.set_output_state(
            self.s, self.s_old, self.s_mid, self.theta0d_timint(t), midpoint=self.output_midpoint
        )
        self.cardvasc0D.set_output_state(
            self.aux, self.aux_old, self.aux_mid, self.theta0d_timint(t), midpoint=self.output_midpoint
        )
    ...
    def write_output(self, N, t):
        ...
        if self.write_results_every_0D > 0 and N % self.write_results_every_0D == 0:
            self.cardvasc0D.write_output(
                self.output_path_0D, t, self.s_mid, self.aux_mid, self.pbase.simname + "_" + self.problem_physics
            )
