...
class SignallingNetworkProblem(problem_base):
    ...
    def set_output_state(self, t):
        ...
        self.signet.set_output_state(
            self.s, self.s_old, self.s_mid, self.thetasn_timint(t), midpoint=self.output_midpoint
        )
        self.signet.set_output_state(
            self.aux, self.aux_old, self.aux_mid, self.thetasn_timint(t), midpoint=self.output_midpoint
        )
    ...
    def write_output(self, N, t):
        ...
        if self.write_results_every_signet > 0 and N % self.write_results_every_signet == 0:
            self.signet.write_output(
                self.output_path_signet, t, self.s_mid, self.aux_mid, self.pbase.simname + "_" + self.problem_physics
            )
